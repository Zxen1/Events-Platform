...
try {
    ...
    if ($includeUserGuide) {
        try {
            ...
            if ($stmt->rowCount() > 0) {
                $stmt = $pdo->query('SELECT `id`, `chapter`, `chapter_key`, `chapter_id`, `title`, `description`, `sort_order` FROM `user_guide` ORDER BY `sort_order` ASC, `id` ASC');
                $rows = $stmt->fetchAll();
                $userGuideItems = [];
                foreach ($rows as $row) {
                    $userGuideItems[] = [
    ...
    if (isset($pdo) && $pdo instanceof PDO) {
        logSiteErrorPdo($pdo, $GLOBALS['FUNMAP_GATEWAY_ACTION'] ?? 'get-admin-settings', $e->getMessage());
    }
    http_response_code(500);
    echo json_encode([
        'success' => false,
