...
try {
    ...
    if ($postId > 0 || $postKey !== '') {
        ...
        $multiSql = "
            ...
            FROM `post_map_cards` mc2
            INNER JOIN `posts` p2 ON p2.id = mc2.post_id
            LEFT JOIN `checkout_options` co2 ON p2.checkout_key = co2.checkout_key
            WHERE mc2.latitude IS NOT NULL AND mc2.longitude IS NOT NULL
              AND (mc2.latitude, mc2.longitude) IN (
