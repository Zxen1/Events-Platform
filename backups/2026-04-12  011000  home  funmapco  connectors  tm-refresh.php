...
foreach ($attractions as $attractionId => $postId) {
    ...
    $cardRes = $mysqli->query(
        "SELECT id, subcategory_key, title, description, venue_name, address_line,
                ...
                latitude, longitude, timezone, ticket_url, session_summary,
                session_start, session_end, price_summary
    ...
    foreach ($validVenueGroups as $vk => $vEvts) {
        ...
        $mysqli->query("UPDATE post_map_cards SET session_start = (SELECT MIN(ps.session_date) FROM post_sessions ps WHERE ps.post_map_card_id = {$cardId}), session_end = (SELECT MAX(ps.session_date) FROM post_sessions ps WHERE ps.post_map_card_id = {$cardId}) WHERE id = {$cardId}");
    }
