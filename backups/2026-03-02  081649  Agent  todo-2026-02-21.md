...
h. **707** `msg_email_post_deletion_warning` — Post deletion 7-day warning — fires at day 23 of the 30-day countdown
i. **708** `msg_email_post_deleted` — Post permanently deleted — fires when cron hard-deletes the post at day 30
...
k. **710** `msg_email_account_deletion_warning` — Account deletion 7-day warning — fires at day 23 of the 30-day countdown
l. **711** `msg_email_account_deleted` — Account permanently deleted — fires when cron hard-deletes the account at day 30 (template exists, cron trigger needed)
m. **712** `msg_email_expiry_warning` — Expiry warning — daily cron job checks posts expiring within 14 days, sends once per member per cycle
n. **713** `msg_email_post_expired` — Post expired — same daily cron job, batched per member
o. **714** `msg_email_inactivity_warning` — Inactivity warning — sent at 335 days of inactivity; warns account closes in 30 days
p. **715** `msg_email_inactivity_closed` — Inactivity closure — sent at 365 days; account closed. Requires `last_login_at` column on `members` and `admins` tables and a daily cron job. **Inactivity definition:** the 365-day clock is based on the most recent of: last login, last post created, last payment, or last post expiry date. If any post is still within its paid period, the account is not inactive. The clock only starts after the last paid post has also expired.
