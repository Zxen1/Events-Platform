...
o. **714** `msg_email_inactivity_warning` — Inactivity warning — sent at 335 days of no login; warns account closes in 30 days
p. **715** `msg_email_inactivity_closed` — Inactivity closure — sent at 365 days; account closed. Requires `last_login_at` column on `members` and `admins` tables and a daily cron job. Logging in at any point resets the clock.
