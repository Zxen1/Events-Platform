...
const PostModule = (function() {
  ...
  function _virtShiftWindow(newStart, options) {
    ...
    if (postListEl.querySelector('.post')) return; // open post locks the window
  ...
  var _GHOST_H = 127; // ghost card height in px — used for index estimation only

  /**
   * Checks if the scroll position has moved outside the current window and swaps if needed.
   */
  function _virtCheckScrollBoundary() {
    ...
    var approxIdx = Math.floor(postListEl.scrollTop / _GHOST_H);
    var targetStart = Math.max(0, approxIdx - 25);
    if (Math.abs(targetStart - _virtStart) > 25) {
      _virtShiftWindow(targetStart);
    }
  }

  /**
   * Debounced settle: after scrolling stops, snap the window to the visible position.
   */
