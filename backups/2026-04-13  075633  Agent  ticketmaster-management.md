...
4. **Collate** — Runs `tm-collate.php` in a loop until no pending rows remain. No API calls.
...
| `cron-ticketmaster-daily.php` | Added all 58 TM countries. Added page rotation (page 0 + rotating deeper pages). Split limits between invocations to stay within API budget. Added import loop (runs until all pending rows are processed). Added refresh as Phase 3. Added cleanup step for expired staging rows. |
