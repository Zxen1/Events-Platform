...
const MapModule = (function() {
  ...
  function lockMultiPostThumbByPostId(postId) {
    var pid = String(postId);
    for (var _lme of mapCardMarkers) {
      var _lmEntry = _lme[1];
      var _lmKey = _lme[0];
      if (!_lmEntry.post.isMultiPost) continue;
      if (!_lmEntry.postIds || !_lmEntry.postIds.includes(pid)) continue;
      var _lmThumbs = _lmEntry.post.locationThumbnails;
      if (!_lmThumbs) continue;
      var _lmUrl = _lmThumbs[pid];
      if (!_lmUrl) continue;
      lockMultiPostThumb(_lmKey, _lmUrl);
      return;
    }
  }
