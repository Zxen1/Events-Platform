  ...
  if ($stmtCard) {
    ...
    if ($writeSessionPricing) {
      if (is_array($sessionsToWrite)) {
        ...
        $mysqli->query("UPDATE post_map_cards SET session_start = (SELECT MIN(ps.session_date) FROM post_sessions ps WHERE ps.post_map_card_id = {$mapCardId}), session_end = (SELECT MAX(ps.session_date) FROM post_sessions ps WHERE ps.post_map_card_id = {$mapCardId}) WHERE id = {$mapCardId}");
      }
