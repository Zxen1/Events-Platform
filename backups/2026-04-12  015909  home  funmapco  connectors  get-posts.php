...
try {
    ...
    if ($memberId <= 0 && $postId <= 0 && $postKey === '') {
        ...
        if ($includeExpired) {
            ...
            $where[] = "(mc.session_end IS NULL OR mc.session_end >= $utcMinus12TodaySql)";
        }
    ...
    $sql = "
        ...
        (!$includeExpired && $memberId <= 0 && $postId <= 0 && $postKey === ''
            ? " AND (mc.session_end IS NULL OR mc.session_end >= $utcMinus12TodaySql)"
            : "") . "
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if ($row['post_map_card_id'] !== null) {
            ...
            $postsById[$postId]['map_cards'][] = [
                ...
                'first_session_date' => $row['session_start'],
