...
- Updated postcard click handler in `posteditor.js` to mirror the PostLocationComponent different-location pattern:
  - `closePanels({ keepMember: true })` + `setMode('map')` → fly → on `moveend` verify arrival → `closePanels({ keepMember: true })` + `setMode('posts')` → `PostModule.openPostById` with `autoExpand`
- Member panel stays open throughout the entire flow

### 9. Fly-To Zoom Centralisation
- Added `flyto_zoom_desktop` (id 319) and `flyto_zoom_mobile` (id 320) to `admin_settings` — both set to 12
- Added admin panel sliders in Map tab (`index.php`) with range 1-18
- Wired sliders in `admin.js` (init + reset)
- Added `flyToZoom` config in `index.js`, synced from DB based on 530px breakpoint
- `MapModule.flyTo` in `map.js` now reads `App.getConfig('flyToZoom')` when no zoom passed (no hardcoded default)
- Removed all hardcoded zoom 14s: geocoder in `map.js`, geolocation (cached + fresh) and search result in `components.js`
- Removed wrong `postsLoadZoom` zoom params from PostLocationComponent (3 call sites in `components.js`)
- All fly-to actions now use the single config value
...
| `index.php` | Added fly-to zoom desktop/mobile sliders in admin Map tab |
| `admin.js` | Wired fly-to zoom sliders (init + reset array) |
| `index.js` | Added `flyToZoom` config, synced from DB (desktop/mobile by 530px breakpoint) |
| `map.js` | `flyTo` reads `flyToZoom` config; removed hardcoded zoom 14 from geocoder |
| `components.js` | Removed `postsLoadZoom` zoom from PostLocationComponent (3 sites); replaced hardcoded zoom 14 in geolocation + search with config |
