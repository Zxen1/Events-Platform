...
const MapModule = (function() {
  ...
  // Multi-post marker colour (blue-900)
  const MULTIPOST_COLOR = '#1e3a8a';
  const MULTIPOST_COLOR_RGB = 'rgb(30, 58, 138)';
  ...
  function getSmallIconUrl(post) {
    ...
    if (post.isMultiPost) {
      if (!post.multiPostDotDataUrl) throw new Error('[Map] Missing multiPostDotDataUrl for multi-post marker (post ID ' + post.id + ')');
      return post.multiPostDotDataUrl;
    }
  ...
  function injectMapCardStyles() {
    ...
    const css = `
      ...
      .map-card-appearance--icon[data-multipost-count]::before {
        width: 36px;
        height: 36px;
      }
      ...
      /* ── MULTI-POST THUMBNAIL CYCLING ────────────────────────────────────────
         Big-card state only: cycling thumbnail replaces the canvas-dot icon.
         The number badge (rendered at 50px, scaled to 30px in hover) floats above.
      ──────────────────────────────────────────────────────────────────────────── */

      /* Hide the canvas-dot icon when the card is hovered or active */
      .map-card-container[data-multipost-count].is-hovered .map-card-icon,
      .map-card-container[data-multipost-count].is-active  .map-card-icon { opacity: 0; }
      @media (hover: hover) and (pointer: fine) {
        .map-card-container[data-multipost-count]:not(.is-active):hover .map-card-icon { opacity: 0; }
      }

      /* Thumbnail cycling wrapper — same anchor as icon, fades in when active */
      .map-card-multipost-thumb {
        position: absolute;
        left: 0;
        top: 0;
        width: ${BIG_ICON_SIZE}px;
        height: ${BIG_ICON_SIZE}px;
        border-radius: 50%;
        transform: translate(-50%, -50%);
        overflow: hidden;
        z-index: 2;
      ...
      .map-card-container.is-active .map-card-multipost-thumb { opacity: 1; }

      /* Two crossfade layers inside the wrapper */
      .map-card-multipost-thumb-a,
      .map-card-multipost-thumb-b {
        position: absolute;
        inset: 0;
        width: 100%;
        height: 100%;
      ...
      /* Number badge: always 50px, scaled to 0.6 in hover, 1.0 in active */
      .map-card-multipost-badge {
        position: absolute;
        left: 0;
        top: 0;
        width: ${BIG_ICON_SIZE}px;
      ...
      .map-card-container.is-hovered .map-card-multipost-badge {
        opacity: 1;
        transform: translate(-50%, -50%) scale(0.6);
      }
      .map-card-container.is-active .map-card-multipost-badge {
        opacity: 1;
        transform: translate(-50%, -50%) scale(1);
      }
      /* Badge hidden once user has selected a post from the list */
      .map-card-container.is-active[data-thumb-locked] .map-card-multipost-badge,
      .map-card-container.is-hovered[data-thumb-locked] .map-card-multipost-badge { opacity: 0; }
      @media (hover: hover) and (pointer: fine) {
        .map-card-container[data-multipost-count]:not(.is-active):hover .map-card-multipost-badge {
          opacity: 1;
          transform: translate(-50%, -50%) scale(0.6);
        }
        .map-card-container[data-thumb-locked]:not(.is-active):hover .map-card-multipost-badge { opacity: 0; }
      }
      .map-card-multipost-badge span {
        color: #fff;
        font-size: 22px;
        font-weight: 700;
  ...
  function onMapLoad() {
    ...
    requestAnimationFrame(function() {
      ...
      App.whenStartupSettingsReady().then(function() {
        ...
        // Initialize ground bubbles
        setupGroundBubbleLayers();

        // Apply filter-based colouring to all bubble layers
        updateBubbleColors();
  ...
  function bindMapEvents() {
    ...
    ['moveend', 'zoomend'].forEach(event => {
      map.on(event, () => {
        ...
        // Update ground bubbles when above breakpoint
        if (zoom >= getClusterZoomMax()) {
          updateGroundBubbleData(zoom, bounds);
        } else {
          setGroundBubbleVisibility(false);
          groundBubbleActive = false;
        }
  ...
  // Cluster constants
  const SKYBUBBLE_SOURCE_ID = 'map-skybubble-source';
  const SKYBUBBLE_LAYER_ID = 'map-skybubble-layer';
  const SKYBUBBLE_LABEL_LAYER_ID = 'map-skybubble-labels';
  const SKYBUBBLE_MIN_ZOOM = 0;

  // Ground bubble constants (low-altitude density clusters)
  const GROUNDBUBBLE_SOURCE_ID = 'map-groundbubble-source';
  const GROUNDBUBBLE_LAYER_ID = 'map-groundbubble-layer';
  const GROUNDBUBBLE_LABEL_LAYER_ID = 'map-groundbubble-labels';
  const GROUNDBUBBLE_THRESHOLD = 100;

  // Ground bubble state
  let groundBubbleActive = false;
  let lastGroundBubbleRequestKey = null;
  let groundBubbleAbort = null;

  // Bubble colour palettes — switches to orange when filters are active
  const BUBBLE_COLOR_DEFAULT = ['interpolate', ['linear'], ['get', 'count'],
    0, '#00e5ff', 25, '#2979ff', 100, '#651fff', 300, '#aa00ff'
  ];
  const BUBBLE_COLOR_FILTERED = ['interpolate', ['linear'], ['get', 'count'],
    0, '#ffaa33', 25, '#ee8800', 100, '#cc6600', 300, '#993d00'
  ];
  const SKYBUBBLE_FILTER_COLOR_ENABLED = true;
  const GROUNDBUBBLE_FILTER_COLOR_ENABLED = true;

  // Balloon constants — disabled (kept for potential reactivation)
  const SKYBUBBLE_BALLOON_LAYER_ID = 'map-skybubble-balloon-layer';
  const SKYBUBBLE_BALLOON_ICON_PREFIX = 'skybubble-balloon-';
  const SKYBUBBLE_BALLOON_MAX_COUNT = 999;
  ...
  function ensureClusterImage(n) {
    var imageId = SKYBUBBLE_BALLOON_ICON_PREFIX + n;
  ...
  function setupClusterLayers() {
    ...
    if (map.getLayer(SKYBUBBLE_LABEL_LAYER_ID)) {
      map.removeLayer(SKYBUBBLE_LABEL_LAYER_ID);
    }
    if (map.getLayer(SKYBUBBLE_LAYER_ID)) {
      map.removeLayer(SKYBUBBLE_LAYER_ID);
    }
    if (map.getLayer(SKYBUBBLE_BALLOON_LAYER_ID)) {
      map.removeLayer(SKYBUBBLE_BALLOON_LAYER_ID);
    }
    
    // Remove existing source if present
    if (map.getSource(SKYBUBBLE_SOURCE_ID)) {
      map.removeSource(SKYBUBBLE_SOURCE_ID);
    }
    
    // Create empty source
    var emptyData = { type: 'FeatureCollection', features: [] };
    map.addSource(SKYBUBBLE_SOURCE_ID, { type: 'geojson', data: emptyData });

    // Proportional bubble layer — zoom 0 through to marker threshold
    map.addLayer({
      id: SKYBUBBLE_LAYER_ID,
      type: 'circle',
      source: SKYBUBBLE_SOURCE_ID,
      minzoom: SKYBUBBLE_MIN_ZOOM,
      ...
      paint: {
        ...
        'circle-color': BUBBLE_COLOR_DEFAULT,
    ...
    // Balloon cluster layer — DISABLED (bubbles now cover this range)
    // map.addLayer({
    //   id: SKYBUBBLE_BALLOON_LAYER_ID,
    //   type: 'symbol',
    //   source: SKYBUBBLE_SOURCE_ID,
    //   minzoom: getMarkerZoomThreshold() - 1,
    //   maxzoom: getClusterZoomMax(),
    //   layout: {
    //     'icon-image': ['concat', SKYBUBBLE_BALLOON_ICON_PREFIX, ['to-string', ['min', ['get', 'count'], SKYBUBBLE_BALLOON_MAX_COUNT]]],
    ...
    // Bind click handlers for sky bubble layer
    map.on('click', SKYBUBBLE_LAYER_ID, handleClusterClick);
    map.on('click', SKYBUBBLE_LABEL_LAYER_ID, handleClusterClick);
    map.on('mouseenter', SKYBUBBLE_LAYER_ID, function() { map.getCanvas().style.cursor = 'pointer'; });
    map.on('mouseleave', SKYBUBBLE_LAYER_ID, function() { map.getCanvas().style.cursor = 'grab'; });
    map.on('mouseenter', SKYBUBBLE_LABEL_LAYER_ID, function() { map.getCanvas().style.cursor = 'pointer'; });
    map.on('mouseleave', SKYBUBBLE_LABEL_LAYER_ID, function() { map.getCanvas().style.cursor = 'grab'; });

    // Balloon click handlers — DISABLED (balloon layer disabled)
    // map.on('click', SKYBUBBLE_BALLOON_LAYER_ID, handleClusterClick);
    // map.on('mouseenter', SKYBUBBLE_BALLOON_LAYER_ID, function() {
    //   if (map.getZoom() >= getClusterZoomMax()) return;
    //   map.getCanvas().style.cursor = 'pointer';
    // });
    // map.on('mouseleave', SKYBUBBLE_BALLOON_LAYER_ID, function() {
  ...
  function updateClusterData(zoom) {
    ...
    var source = map.getSource(SKYBUBBLE_SOURCE_ID);
    ...
    fetchClusterData(zoomValue, clusterAbort ? clusterAbort.signal : null)
      .then(function(result) {
        ...
        // Balloon image stamping — not needed while balloon layer is disabled
  ...
  function hasActiveFilters() {
    try {
      ...
      if (f.keyword || f.minPrice || f.maxPrice || f.dateStart || f.dateEnd
        || f.expired || f.show18Plus
        || (f.amenities && typeof f.amenities === 'object' && Object.keys(f.amenities).length > 0)) {
        return true;
      }
      if (Array.isArray(f.solo) && f.solo.length > 0) return true;
      if (f.categories && typeof f.categories === 'object') {
        var cats = f.categories;
        var keys = Object.keys(cats);
        for (var i = 0; i < keys.length; i++) {
          var cat = cats[keys[i]];
          if (!cat.enabled) return true;
          if (cat.subs && typeof cat.subs === 'object') {
            var subKeys = Object.keys(cat.subs);
            for (var j = 0; j < subKeys.length; j++) {
              if (!cat.subs[subKeys[j]]) return true;
            }
          }
        }
      }
  ...
  function updateBubbleColors() {
    ...
    if (filtered) {
      if (SKYBUBBLE_FILTER_COLOR_ENABLED && map.getLayer(SKYBUBBLE_LAYER_ID)) {
        map.setPaintProperty(SKYBUBBLE_LAYER_ID, 'circle-color', BUBBLE_COLOR_FILTERED);
        map.setPaintProperty(SKYBUBBLE_LAYER_ID, 'circle-emissive-strength', 1);
        map.setPaintProperty(SKYBUBBLE_LAYER_ID, 'circle-stroke-opacity', 0.3);
      }
      if (GROUNDBUBBLE_FILTER_COLOR_ENABLED && map.getLayer(GROUNDBUBBLE_LAYER_ID)) {
        map.setPaintProperty(GROUNDBUBBLE_LAYER_ID, 'circle-color', BUBBLE_COLOR_FILTERED);
        map.setPaintProperty(GROUNDBUBBLE_LAYER_ID, 'circle-emissive-strength', 1);
        map.setPaintProperty(GROUNDBUBBLE_LAYER_ID, 'circle-stroke-opacity', 0.3);
      }
    } else {
      if (SKYBUBBLE_FILTER_COLOR_ENABLED && map.getLayer(SKYBUBBLE_LAYER_ID)) {
        map.setPaintProperty(SKYBUBBLE_LAYER_ID, 'circle-color', BUBBLE_COLOR_DEFAULT);
        map.setPaintProperty(SKYBUBBLE_LAYER_ID, 'circle-emissive-strength', 0);
        map.setPaintProperty(SKYBUBBLE_LAYER_ID, 'circle-stroke-opacity', 1);
      }
      if (GROUNDBUBBLE_FILTER_COLOR_ENABLED && map.getLayer(GROUNDBUBBLE_LAYER_ID)) {
        map.setPaintProperty(GROUNDBUBBLE_LAYER_ID, 'circle-color', BUBBLE_COLOR_DEFAULT);
        map.setPaintProperty(GROUNDBUBBLE_LAYER_ID, 'circle-emissive-strength', 0);
        map.setPaintProperty(GROUNDBUBBLE_LAYER_ID, 'circle-stroke-opacity', 1);
      }
    }
  }
  ...
  function buildClusterFeatureCollectionFromServer(clusters) {
    ...
    var features = clusters.map(function(c) {
      // var count = Math.min(c.count, SKYBUBBLE_BALLOON_MAX_COUNT);
      var count = c.count;
      return {
        type: 'Feature',
        properties: {
  ...
  function updateClusterVisibility(zoom) {
    ...
    if (shouldShow !== clusterLayerVisible) {
      ...
      // Balloon visibility toggle — not needed while balloon layer is disabled
      // if (map.getLayer(SKYBUBBLE_BALLOON_LAYER_ID)) {
      //   map.setLayoutProperty(SKYBUBBLE_BALLOON_LAYER_ID, 'visibility', shouldShow ? 'visible' : 'none');
      // }
    }
  ...
  /* ==========================================================================
     SECTION 5b: GROUND BUBBLES - Low-altitude density clusters
     When total results in viewport >= GROUNDBUBBLE_THRESHOLD, these appear
     alongside the admin-selected map cards instead of individual markers.
     ========================================================================== */

  function setupGroundBubbleLayers() {
    if (!map) return;

    if (map.getLayer(GROUNDBUBBLE_LABEL_LAYER_ID)) {
      map.removeLayer(GROUNDBUBBLE_LABEL_LAYER_ID);
    }
    if (map.getLayer(GROUNDBUBBLE_LAYER_ID)) {
      map.removeLayer(GROUNDBUBBLE_LAYER_ID);
    }
    if (map.getSource(GROUNDBUBBLE_SOURCE_ID)) {
      map.removeSource(GROUNDBUBBLE_SOURCE_ID);
    }

    var emptyData = { type: 'FeatureCollection', features: [] };
    map.addSource(GROUNDBUBBLE_SOURCE_ID, { type: 'geojson', data: emptyData });

    map.addLayer({
      id: GROUNDBUBBLE_LAYER_ID,
      type: 'circle',
      source: GROUNDBUBBLE_SOURCE_ID,
      minzoom: getClusterZoomMax(),
      paint: {
        'circle-radius': ['interpolate', ['linear'], ['get', 'count'],
          1, 14, 10, 22, 50, 34, 200, 48
        ],
        'circle-color': BUBBLE_COLOR_DEFAULT,
        'circle-opacity': 0.8,
        'circle-stroke-width': 2.5,
    ...
    map.addLayer({
      id: GROUNDBUBBLE_LABEL_LAYER_ID,
      type: 'symbol',
      source: GROUNDBUBBLE_SOURCE_ID,
      minzoom: getClusterZoomMax(),
      layout: {
    ...
    map.on('click', GROUNDBUBBLE_LAYER_ID, handleGroundBubbleClick);
    map.on('click', GROUNDBUBBLE_LABEL_LAYER_ID, handleGroundBubbleClick);
    map.on('mouseenter', GROUNDBUBBLE_LAYER_ID, function() { map.getCanvas().style.cursor = 'pointer'; });
    map.on('mouseleave', GROUNDBUBBLE_LAYER_ID, function() { map.getCanvas().style.cursor = 'grab'; });
    map.on('mouseenter', GROUNDBUBBLE_LABEL_LAYER_ID, function() { map.getCanvas().style.cursor = 'pointer'; });
    map.on('mouseleave', GROUNDBUBBLE_LABEL_LAYER_ID, function() { map.getCanvas().style.cursor = 'grab'; });
  }

  function handleGroundBubbleClick(e) {
    if (!e || !e.features || !e.features[0]) return;
    var coords = e.features[0].geometry.coordinates;
    var currentZoom = map.getZoom();
    map.flyTo({
  ...
  /**
   * Fetch ground bubble cluster data for the current viewport.
   * Called when zoom >= breakpoint and the viewport moves.
   */
  function updateGroundBubbleData(zoom, bounds) {
    // TEST: Disable ground bubbles to measure loading speed without them
    return;
    if (!map) return;

    var source = map.getSource(GROUNDBUBBLE_SOURCE_ID);
    if (!source || typeof source.setData !== 'function') return;
    ...
    var gridSize = getClusterGridSize(zoom);
    var filterKey = getClusterFilterKey();
    var requestKey = gridSize.toFixed(3) + '|' + boundsStr + '|' + filterKey;

    if (lastGroundBubbleRequestKey === requestKey) return;

    try { if (groundBubbleAbort) groundBubbleAbort.abort(); } catch (_e) {}
    groundBubbleAbort = (typeof AbortController !== 'undefined') ? new AbortController() : null;

    var qs = new URLSearchParams();
    qs.set('action', 'get-clusters');
    qs.set('zoom', String(zoom));
    qs.set('bounds', boundsStr);
    ...
    try {
      ...
      if (raw) {
        ...
        if (saved && typeof saved === 'object') {
          ...
          if (saved.expired) qs.set('expired', '1');
          if (saved.show18Plus) qs.set('show18_plus', '1');
          if (Array.isArray(saved.subcategoryKeys) && saved.subcategoryKeys.length > 0) {
            qs.set('subcategory_keys', saved.subcategoryKeys.map(String).join(','));
          }
          if (saved.amenities && typeof saved.amenities === 'object' && Object.keys(saved.amenities).length > 0) {
            qs.set('amenities', JSON.stringify(saved.amenities));
    ...
    var myKey = requestKey;
    fetch('/gateway.php?' + qs.toString(), groundBubbleAbort ? { signal: groundBubbleAbort.signal } : undefined)
      .then(function(response) { return response.json(); })
      .then(function(data) {
        if (myKey !== requestKey) return;
        var clusters = (data.success && Array.isArray(data.clusters)) ? data.clusters : [];
        var totalCount = (typeof data.total_count === 'number') ? data.total_count : 0;

        var shouldActivate = totalCount >= GROUNDBUBBLE_THRESHOLD;

        if (shouldActivate) {
          var features = clusters.map(function(c) {
            return {
              type: 'Feature',
              properties: { count: c.count, label: c.label },
              geometry: { type: 'Point', coordinates: [c.lng, c.lat] }
            };
          });
        ...
        setGroundBubbleVisibility(shouldActivate);

        if (shouldActivate !== groundBubbleActive) {
          groundBubbleActive = shouldActivate;
          App.emit('groundBubbles:toggled', { active: shouldActivate, total: totalCount });
        }

        lastGroundBubbleRequestKey = myKey;
      })
      .catch(function(err) {
        try {
          if (err && (err.name === 'AbortError' || String(err.message || '').toLowerCase().indexOf('abort') !== -1)) return;
        } catch (_e) {}
        console.error('[Map] Ground bubble fetch failed:', err);
      });
  }

  function refreshGroundBubbles() {
    if (!map) return;
    var zoom = map.getZoom() || 0;
    if (zoom < getClusterZoomMax()) {
      groundBubbleActive = false;
    ...
    lastGroundBubbleRequestKey = null;
    updateGroundBubbleData(zoom, map.getBounds());
  }

  function setGroundBubbleVisibility(visible) {
    if (!map) return;
    if (map.getLayer(GROUNDBUBBLE_LAYER_ID)) {
      map.setLayoutProperty(GROUNDBUBBLE_LAYER_ID, 'visibility', visible ? 'visible' : 'none');
    }
    if (map.getLayer(GROUNDBUBBLE_LABEL_LAYER_ID)) {
      map.setLayoutProperty(GROUNDBUBBLE_LABEL_LAYER_ID, 'visibility', visible ? 'visible' : 'none');
    }
  }
  ...
  function generateMultiPostDotDataUrl(count, color) {
    ...
    var cx = SIZE / 2, cy = SIZE / 2, r = SIZE / 2 - 0.5;
  ...
  function createMapCardMarker(post, lng, lat, appearance, dotColor) {
    ...
    if (post.isMultiPost) {
      ...
      el.style.setProperty('--subcat-color', MULTIPOST_COLOR);
      el.style.setProperty('--pill-fill', MULTIPOST_COLOR_RGB);
      if (!post.multiPostDotDataUrl) {
        post.multiPostDotDataUrl = generateMultiPostDotDataUrl(_mpCount, MULTIPOST_COLOR);
      }
    ...
    if (post.isMultiPost && post.locationPostCount > 1 && appearance !== 'card') {
      ...
      _dotImg.src = post.multiPostDotDataUrl;
      _dotImg.width  = 30;
      _dotImg.height = 30;
      _dotImg.alt = '';
  ...
  function buildMapCardHTML(post, state) {
    ...
    const iconEl = post.isMultiPost
      ? `<img class="map-card-icon" src="${post.multiPostDotDataUrl}" width="${iconSize}" height="${iconSize}" alt="">`
      : `<img class="map-card-icon" src="${iconUrl}" width="${iconSize}" height="${iconSize}" alt="">`;

    const multiPostExtras = post.isMultiPost ? `
      <div class="map-card-multipost-thumb">
        <img class="map-card-multipost-thumb-a" src="" alt="">
        <img class="map-card-multipost-thumb-b" src="" alt="">
      </div>
      <div class="map-card-multipost-badge"><span>${escapeHtml(String(post.locationPostCount || ''))}</span></div>
    ` : '';
  ...
  function onMapCardClick(locationKey) {
    ...
    if (entry.state === 'big') {
      if (entry.post && entry.post.isMultiPost) {
        ...
        // Close whichever post is actually open — may differ from entry.post.id for multipost
        if (window.PostModule && typeof PostModule.closePost === 'function') {
          var _openEl = document.querySelector('.post[data-id]');
          if (_openEl && _openEl.dataset && _openEl.dataset.id) {
            PostModule.closePost(_openEl.dataset.id);
          }
        }
      } else {
        if (window.PostModule && typeof PostModule.closePost === 'function') {
  ...
  // Registry of active thumbnail cycling timers: locationKey -> { cancel }
  var _multiPostCycles = new Map();

  /**
   * Start cycling thumbnails on a multi-post big card.
   * Preloads each next image before swapping. Holds on current if next isn't ready.
   */
  function startMultiPostCycle(entry) {
    stopMultiPostCycle(entry.locationKey);
    delete entry.element.dataset.thumbLocked;

    var thumbsObj = entry.post.locationThumbnails;
    var thumbs = thumbsObj ? Object.values(thumbsObj) : [];
    var thumbEl = entry.element.querySelector('.map-card-multipost-thumb');
    if (!thumbEl) return;
    var imgA = thumbEl.querySelector('.map-card-multipost-thumb-a');
    var imgB = thumbEl.querySelector('.map-card-multipost-thumb-b');
    if (!imgA || !imgB) return;
    if (!thumbs.length) return;

    // Single thumbnail: show it, no cycling needed
    if (thumbs.length === 1) {
      imgA.src = thumbs[0];
    ...
    function preloadAndSwap(nextIdx) {
      if (cancelled) return;
      var preload = new Image();
      preload.onload = function() {
        if (cancelled) return;
        if (aIsTop) {
          imgB.src = preload.src;
          imgA.style.opacity = '0';
          imgB.style.opacity = '1';
          aIsTop = false;
        } else {
          imgA.src = preload.src;
          imgB.style.opacity = '0';
          imgA.style.opacity = '1';
          aIsTop = true;
        }
        ...
        timer = setTimeout(function() {
          if (!cancelled) preloadAndSwap((currentIdx + 1) % thumbs.length);
        }, 2000);
      };
      preload.src = thumbs[nextIdx];
    }

    // Show first thumbnail immediately, then schedule the first swap
    imgA.src = thumbs[0];
    imgA.style.opacity = '1';
    imgB.style.opacity = '0';
    aIsTop = true;
    ...
    timer = setTimeout(function() {
      if (!cancelled) preloadAndSwap(1 % thumbs.length);
    }, 2000);

    _multiPostCycles.set(entry.locationKey, {
      cancel: function() {
        cancelled = true;
  ...
  /**
   * Stop thumbnail cycling for a marker.
   */
  function stopMultiPostCycle(locationKey) {
    var cycle = _multiPostCycles.get(locationKey);
    if (cycle) { cycle.cancel(); _multiPostCycles.delete(locationKey); }
  }

  /**
   * Stop cycling and lock the thumbnail to a specific image (called on post selection).
   */
  function lockMultiPostThumb(locationKey, thumbnailUrl) {
    stopMultiPostCycle(locationKey);
    var entry = mapCardMarkers.get(locationKey);
    if (!entry) return;
    var thumbEl = entry.element.querySelector('.map-card-multipost-thumb');
    if (!thumbEl) return;
    var imgA = thumbEl.querySelector('.map-card-multipost-thumb-a');
    ...
    // Mark container so CSS hides the number badge (post has been chosen)
    entry.element.dataset.thumbLocked = '1';

    // Load into whichever layer is currently hidden, then crossfade
    var aIsVisible = (imgA.style.opacity !== '0');
    var showLayer = aIsVisible ? imgB : imgA;
    var hideLayer = aIsVisible ? imgA : imgB;

    var preload = new Image();
    preload.onload = function() {
      showLayer.src = thumbnailUrl;
  ...
  /**
   * Start cycling for the marker at a given locationKey.
   * Called explicitly by PostModule when the multipost modal opens.
   */
  function startMultiPostCycleByLocationKey(locationKey) {
    var entry = mapCardMarkers.get(locationKey);
    if (!entry || !entry.post.isMultiPost) return;
    startMultiPostCycle(entry);
  }

  /**
   * Find any multipost marker containing postId and lock its thumbnail to that post.
   * Called by PostModule whenever a post is opened (post panel, direct link, etc.).
   */
  function lockMultiPostThumbByPostId(postId) {
    const pid = String(postId);
    for (const [locationKey, entry] of mapCardMarkers) {
      if (!entry.post.isMultiPost) continue;
      if (!entry.postIds || !entry.postIds.includes(pid)) continue;
      const thumbsObj = entry.post.locationThumbnails;
      if (!thumbsObj) continue;
      const url = thumbsObj[pid];
      if (!url) continue;
      lockMultiPostThumb(locationKey, url);
      return;
    }
  }

  /**
   * Lock the thumbnail to the post with the given ID (looks up URL from marker data).
   * Called by PostModule when a post is selected from the multipost modal.
   */
  function lockMultiPostThumbForPostId(locationKey, postId) {
    var entry = mapCardMarkers.get(locationKey);
    if (!entry) return;
    var thumbsObj = entry.post.locationThumbnails;
    if (!thumbsObj) return;
    var url = thumbsObj[String(postId)];
    if (!url) return;
    lockMultiPostThumb(locationKey, url);
  }
  ...
  function updateMapCardStateByKey(locationKey, newState) {
    ...
    // Multi-post thumbnail cycling: stop cycle when leaving big state
    // (Cycle is started explicitly by openMultipostModal, not here)
    if (entry.post.isMultiPost && newState !== 'big') {
      stopMultiPostCycle(entry.locationKey);
    }
  ...
  return {
    ...
    lockMultiPostThumb,
    lockMultiPostThumbForPostId,
    lockMultiPostThumbByPostId,
    startMultiPostCycleByLocationKey,
    ...
    // Ground bubbles
    refreshGroundBubbles,
    isGroundBubbleActive: function() { return groundBubbleActive; },
    getGroundBubbleThreshold: function() { return GROUNDBUBBLE_THRESHOLD; },
