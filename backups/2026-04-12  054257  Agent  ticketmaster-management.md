...
**Deferred — batch multiple attraction IDs per API call.** Needs verification that TM API supports comma-separated `attractionId`. Can revisit if budget becomes tight.
...
Need to determine how Ticketmaster defines their 24-hour rate limit period — is it a rolling window from first call, or a calendar day reset? This affects cron scheduling. If rolling, all API calls must land in a tight window to avoid oscillation between runs. The new phase ordering (collect → refresh back-to-back) mitigates this regardless.
