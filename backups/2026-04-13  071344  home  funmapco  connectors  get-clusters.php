...
try {
    ...
    if ($includeExpired) {
        ...
        $where[] = "(pmc.end_date IS NULL OR pmc.end_date >= $utcMinus12TodaySql)";
    }
    ...
    // Bounds filter (optional — used by ground bubbles for viewport-scoped clusters)
    $bounds = null;
    if (!empty($_GET['bounds'])) {
        ...
        if (count($parts) === 4) {
            ...
            $where[] = 'pmc.latitude BETWEEN ? AND ?';
            $where[] = 'pmc.longitude BETWEEN ? AND ?';
