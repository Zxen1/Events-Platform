 ...
 * - Never uses post_map_cards price_summary/amenity_summary for filtering.
 */
...
try {
  ...
  if ($includeExpired) {
    ...
    $paramsBaseline[] = 'active';
    $typesBaseline .= 's';
    $whereBaseline[] = "(pmc.end_date IS NULL OR pmc.end_date >= $utcMinus12TodaySql)";
  }
  ...
  // Exclude map cards when ANY age source is 18+ unless show18Plus is set.
  if (!$show18Plus) {
    $whereScope[] = "(
  ...
  foreach ($amenitiesFilter as $amenityKey => $amenityVal) {
    ...
    if ($amenityVal === 'yes') {
      ...
      $whereFiltered[] = 'EXISTS (SELECT 1 FROM post_amenities pa WHERE pa.post_map_card_id = pmc.id AND pa.amenity_key = ? AND pa.value = 0)';
      $paramsFiltered[] = $amenityKey;
      $typesFiltered .= 's';
    }
  }
