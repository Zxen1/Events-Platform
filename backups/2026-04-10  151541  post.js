...
const PostModule = (function() {
  ...
  function renderMapMarkers(posts) {
    ...
    Object.keys(locationGroups).forEach(function(locationKey) {
      ...
      if (isStorefront) {
        ...
        // Collect one thumbnail per post in group order (used for big-card cycling)
        markerData.locationThumbnails = group.map(function(gi) {
          var _rawUrl = (gi.mapCard && gi.mapCard.media_urls && gi.mapCard.media_urls.length) ? gi.mapCard.media_urls[0] : '';
          return _rawUrl ? addImageClass(_rawUrl, 'minithumb') : '';
        }).filter(Boolean);
      }
  ...
  function _lockMultiPostThumbForPost(data, post) {
    if (!window.MapModule || typeof MapModule.lockMultiPostThumb !== 'function') return;
    var locationKey = (data && data.locationKey) ? data.locationKey : _multipostModalLocationKey;
    if (!locationKey) return;
    var rawMedia = (post.map_cards && post.map_cards[0] && post.map_cards[0].media_urls && post.map_cards[0].media_urls.length) ? post.map_cards[0].media_urls[0] : '';
    if (!rawMedia) return;
    MapModule.lockMultiPostThumb(locationKey, addImageClass(rawMedia, 'minithumb'));
  }
