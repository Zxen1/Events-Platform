...
const PostModule = (function() {
  ...
  function _virtRenderWindow(forceFullRender) {
    ...
    currentEls.forEach(function(el) {
      if (el.querySelector('.post')) {
        pinnedEl = el;
        var mc = el.querySelector('.post-main-container');
        var pinnedPostId = mc && mc.dataset ? mc.dataset.id : '';
        if (pinnedPostId) {
          for (var pi = 0; pi < _virtItems.length; pi++) {
            if (String(_virtItems[pi].post.id) === pinnedPostId) { pinnedIdx = pi; break; }
          }
        }
        return;
      }
  ...
  function _mpVirtRenderWindow(forceFullRender) {
    ...
    var frag = document.createDocumentFragment();
    for (var i = newStart; i < newEnd; i++) {
      frag.appendChild(_mpVirtCreateCard(_mpVirtPosts[i], _mpVirtData));
    }
