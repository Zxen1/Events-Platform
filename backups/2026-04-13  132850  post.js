...
const PostModule = (function() {
  ...
  // Virtualisation: the in-memory array of virtualised items.
  // The DOM only contains ~50 real postcard elements at any time.
  var _virtItems = [];
  var _virtWindowStart = 0;
  var _virtWindowEnd = 0;
  var _virtGhostAbove = null;
  var _virtGhostBelow = null;
  var _virtGhostCardHeight = 0;
  var _virtScrollBound = false;
  var _virtRafPending = false;
  var _virtPreRendered = null;
  var _VIRT_BUFFER = 20;
  ...
  var _mpVirtPosts = [];
  var _mpVirtData = null;
  var _mpVirtWindowStart = -1;
  var _mpVirtWindowEnd = -1;
  var _mpVirtGhostAbove = null;
  var _mpVirtGhostBelow = null;
  var _mpVirtRafPending = false;
  ...
  function _virtBuildItems(posts, sfLookup, sfGroups) {
    var items = [];
    var sfRendered = {};
    for (var pi = 0; pi < posts.length; pi++) {
      var post = posts[pi];
      var sfKey = sfLookup[post.id];
      if (sfKey) {
        if (sfRendered[sfKey]) continue;
        sfRendered[sfKey] = true;
        var sfGroup = sfGroups[sfKey];
        if (sfGroup[0].id !== post.id) continue;
        var pick = pickMapCardInCurrentBounds(sfGroup[0]);
        var mc = pick.mapCard;
        var sfHighest = sfGroup.reduce(function(best, p) {
          return (p.checkout_sort_order || 0) > (best.checkout_sort_order || 0) ? p : best;
        }, sfGroup[0]);
        items.push({
          type: 'storefront',
          ...
          sfPosts: sfGroup,
          sortData: {
            checkoutOrder: sfHighest.checkout_sort_order || 0,
            title: ('storefront: ' + (sfGroup[0].member_name || '')).toLowerCase(),
            createdAt: new Date(sfGroup[0].created_at || 0).getTime() || 0,
            price: extractPrice(mc),
            soonTs: '',
            lng: (mc && Number.isFinite(Number(mc.longitude))) ? Number(mc.longitude) : NaN,
            lat: (mc && Number.isFinite(Number(mc.latitude))) ? Number(mc.latitude) : NaN,
            sfIds: sfGroup.map(function(p) { return String(p.id); })
          }
        });
      } else {
        var pickN = pickMapCardInCurrentBounds(post);
        var mcN = pickN.mapCard;
        var title = (mcN && mcN.title) || post.checkout_title || post.title || '';
        if (title === 'Array') title = 'Post #' + post.id;
        items.push({
          type: 'post',
          post: post,
          sfPosts: null,
          sortData: {
            checkoutOrder: post.checkout_sort_order || 0,
            title: String(title || '').toLowerCase(),
            createdAt: new Date(post.created_at || 0).getTime() || 0,
            price: extractPrice(mcN),
            soonTs: String(getMapCardSoonestTimestamp(mcN) || ''),
            lng: (mcN && Number.isFinite(Number(mcN.longitude))) ? Number(mcN.longitude) : NaN,
            lat: (mcN && Number.isFinite(Number(mcN.latitude))) ? Number(mcN.latitude) : NaN,
            sfIds: null
          }
        });
      }
  ...
  function _virtSortItems(sortKey, geoLocation) {
    if (!_virtItems || !_virtItems.length) return;
    var nearestOrigin = geoLocation || null;
    _virtItems.sort(function(a, b) {
      if (favToTop && !favSortDirty) {
        var favA = false, favB = false;
        if (a.sortData.sfIds) {
          favA = a.sortData.sfIds.some(function(id) { return isFavorite(id); });
        } else {
          favA = isFavorite(a.post.id);
        }
        if (b.sortData.sfIds) {
          favB = b.sortData.sfIds.some(function(id) { return isFavorite(id); });
        } else {
          favB = isFavorite(b.post.id);
        }
        if (favA !== favB) return (favB ? 1 : 0) - (favA ? 1 : 0);
      }
      var ad = a.sortData, bd = b.sortData;
      switch (sortKey) {
        case 'recommended':
          if (ad.checkoutOrder !== bd.checkoutOrder) return bd.checkoutOrder - ad.checkoutOrder;
          return (bd.createdAt || 0) - (ad.createdAt || 0);
        case 'az':
          return String(ad.title || '').localeCompare(String(bd.title || ''));
        case 'za':
          return String(bd.title || '').localeCompare(String(ad.title || ''));
        case 'newest':
          return (bd.createdAt || 0) - (ad.createdAt || 0);
        case 'oldest':
          return (ad.createdAt || 0) - (bd.createdAt || 0);
        case 'price-low':
          return (ad.price || 0) - (bd.price || 0);
        case 'price-high':
          return (bd.price || 0) - (ad.price || 0);
        case 'nearest':
          if (!nearestOrigin) return 0;
          var dA = (Number.isFinite(ad.lng) && Number.isFinite(ad.lat))
            ? distKm({ lng: ad.lng, lat: ad.lat }, { lng: nearestOrigin.lng, lat: nearestOrigin.lat })
            : Number.POSITIVE_INFINITY;
          var dB = (Number.isFinite(bd.lng) && Number.isFinite(bd.lat))
            ? distKm({ lng: bd.lng, lat: bd.lat }, { lng: nearestOrigin.lng, lat: nearestOrigin.lat })
            : Number.POSITIVE_INFINITY;
          return dA - dB;
        case 'soon':
          return (Number(ad.soonTs) || Number.POSITIVE_INFINITY) - (Number(bd.soonTs) || Number.POSITIVE_INFINITY);
        default:
          return 0;
      }
    });
  ...
  function _virtFillSlot(outerEl, item) {
    outerEl.innerHTML = '';
    var card;
    if (item.type === 'storefront') {
      card = renderStorefrontCard(item.sfPosts);
    } else {
      card = renderPostCard(item.post);
    }
    var anchor = document.createElement('div');
    anchor.setAttribute('data-slack-anchor', '');
    anchor.appendChild(card);
    var mainSlot = document.createElement('div');
    mainSlot.className = 'post-main-container';
    mainSlot.dataset.id = String(item.post.id);
    if (item.type === 'storefront' && item.sortData.sfIds) {
      mainSlot.dataset.sfIds = item.sortData.sfIds.join(',');
    }
    mainSlot.appendChild(anchor);
    if (item.type !== 'storefront') {
      var sett = App.getState('settings') || {};
      if (sett.countdown_postcards) {
        var pick = pickMapCardInCurrentBounds(item.post);
        var barResult = buildCountdownStatusBar(item.post, pick.mapCard);
        if (barResult) {
          barResult.bar.classList.add('post-statusbar--slot-card');
          if (sett.countdown_postcards_mode === 'soonest_only') {
            barResult.bar.classList.add('post-statusbar--modesoonest');
          }
          card.classList.add('post-card--countdown' + barResult.state);
          var statusCont = document.createElement('div');
          statusCont.className = 'post-status-container';
          statusCont.appendChild(barResult.bar);
          outerEl.appendChild(statusCont);
        }
      }
    }
    outerEl.appendChild(mainSlot);
  }

  function _virtCreateSlot(item) {
    var outerEl = document.createElement('div');
    outerEl.className = 'post-outer-container';
    _virtFillSlot(outerEl, item);
    return outerEl;
  }

  function _virtRenderWindow(forceFullRender) {
    if (!_virtItems || !_virtItems.length || !postListEl) return;
    var totalItems = _virtItems.length;
    var viewportHeight = postListEl.clientHeight || 600;
    var gcH = _virtGhostCardHeight || 110;
    var visibleCount = Math.max(1, Math.ceil(viewportHeight / gcH));
    var scrollTop = postListEl.scrollTop;
    var ghostAboveH = _virtGhostAbove ? (parseFloat(_virtGhostAbove.style.height) || 0) : 0;
    var scrollPastGhostAbove = scrollTop - ghostAboveH;
    var itemsInGhost = _virtWindowStart > 0 ? _virtWindowStart : 0;
    var approxIdx;
    if (scrollPastGhostAbove <= 0) {
      approxIdx = Math.max(0, Math.floor(scrollTop / gcH));
    } else {
      approxIdx = itemsInGhost + Math.floor(scrollPastGhostAbove / gcH);
    }
    approxIdx = Math.max(0, Math.min(totalItems - 1, approxIdx));
    var windowSize = visibleCount + _VIRT_BUFFER * 2;
    var newStart = Math.max(0, approxIdx - _VIRT_BUFFER);
    var newEnd = Math.min(totalItems, newStart + windowSize);
    if (newEnd === totalItems && newEnd - newStart < windowSize) {
      newStart = Math.max(0, newEnd - windowSize);
    }
    if (newStart === 0 && newEnd - newStart < windowSize) {
      newEnd = Math.min(totalItems, newStart + windowSize);
    }
    if (!forceFullRender && newStart === _virtWindowStart && newEnd === _virtWindowEnd) return;
    if (!forceFullRender && postListEl.querySelector('.post')) return;

    var prevStart = _virtWindowStart;
    var prevEnd = _virtWindowEnd;

    if (forceFullRender || prevStart === -1) {
      // Full rebuild: initial render, sort change, filter change
      var currentEls;
      try {
        currentEls = Array.prototype.slice.call(postListEl.querySelectorAll(':scope > .post-outer-container'));
      } catch (_e) {
        currentEls = Array.prototype.slice.call(postListEl.querySelectorAll('.post-outer-container'));
        currentEls = currentEls.filter(function(s) { return s.parentElement === postListEl; });
      }
      currentEls.forEach(function(el) {
        try { postListEl.removeChild(el); } catch (_e2) {}
      });

      var frag = document.createDocumentFragment();
      for (var i = newStart; i < newEnd; i++) {
        var el;
        if (_virtPreRendered && _virtPreRendered[i]) {
          el = _virtPreRendered[i];
          delete _virtPreRendered[i];
        } else {
          el = _virtCreateSlot(_virtItems[i]);
        }
        el._virtIdx = i;
        frag.appendChild(el);
      }

      if (_virtGhostBelow && _virtGhostBelow.parentElement === postListEl) {
        postListEl.insertBefore(frag, _virtGhostBelow);
      } else {
        var botSlack = postListEl.querySelector('.bottomSlack');
        if (botSlack) {
          postListEl.insertBefore(frag, botSlack);
        } else {
          postListEl.appendChild(frag);
        }
      }
    } else {
      // Incremental update: only add/remove at edges, in-window cards stay in place
      var cardsByIdx = {};
      var existingEls;
      try {
        existingEls = Array.prototype.slice.call(postListEl.querySelectorAll(':scope > .post-outer-container'));
      } catch (_eQ) {
        existingEls = Array.prototype.slice.call(postListEl.querySelectorAll('.post-outer-container'));
        existingEls = existingEls.filter(function(s) { return s.parentElement === postListEl; });
      }
      existingEls.forEach(function(el) {
        if (typeof el._virtIdx === 'number') cardsByIdx[el._virtIdx] = el;
      });

      // Remove cards that left the window
      var idxKeys = Object.keys(cardsByIdx);
      for (var ri = 0; ri < idxKeys.length; ri++) {
        var rIdx = Number(idxKeys[ri]);
        if (rIdx < newStart || rIdx >= newEnd) {
          try { postListEl.removeChild(cardsByIdx[rIdx]); } catch (_eRm) {}
          delete cardsByIdx[rIdx];
        }
      }

      // Add cards that entered the window (inserted in correct DOM order)
      for (var ai = newStart; ai < newEnd; ai++) {
        if (cardsByIdx[ai]) continue;
        var newEl = _virtCreateSlot(_virtItems[ai]);
        newEl._virtIdx = ai;
        // Find the next existing card after this index to insert before
        var insertRef = null;
        for (var si = ai + 1; si < newEnd; si++) {
          if (cardsByIdx[si]) { insertRef = cardsByIdx[si]; break; }
        }
        if (insertRef) {
          postListEl.insertBefore(newEl, insertRef);
        } else if (_virtGhostBelow && _virtGhostBelow.parentElement === postListEl) {
          postListEl.insertBefore(newEl, _virtGhostBelow);
        } else {
    ...
    if (_virtGhostAbove) _virtGhostAbove.style.height = (newStart * gcH) + 'px';
    if (_virtGhostBelow) _virtGhostBelow.style.height = ((totalItems - newEnd) * gcH) + 'px';
  }

  function _virtOnScroll() {
    if (_virtRafPending) return;
    _virtRafPending = true;
    requestAnimationFrame(function() {
      _virtRafPending = false;
      _virtRenderWindow(false);
    });
  }

  /* --------------------------------------------------------------------------
     MULTIPOST MODAL — VIRTUALISED SCROLLING
     -------------------------------------------------------------------------- */

  function _mpVirtCreateCard(post, data) {
    ...
    card.addEventListener('click', function(e) {
      ...
      _lockMultiPostThumbForPost(data, post);
  ...
  function _mpVirtRenderWindow(forceFullRender) {
    if (!_mpVirtPosts || !_mpVirtPosts.length || !_multipostModalEl) return;
    var body = _multipostModalEl.querySelector('.multipost-modal-body');
    if (!body) return;

    var totalItems = _mpVirtPosts.length;
    var gcH = _virtGhostCardHeight || 110;
    var viewportHeight = body.clientHeight || 400;
    var visibleCount = Math.max(1, Math.ceil(viewportHeight / gcH));
    var scrollTop = body.scrollTop;
    var ghostAboveH = _mpVirtGhostAbove ? (parseFloat(_mpVirtGhostAbove.style.height) || 0) : 0;
    var scrollPastGhostAbove = scrollTop - ghostAboveH;
    var itemsInGhost = _mpVirtWindowStart > 0 ? _mpVirtWindowStart : 0;
    var approxIdx;
    if (scrollPastGhostAbove <= 0) {
      approxIdx = Math.max(0, Math.floor(scrollTop / gcH));
    } else {
    ...
    if (!forceFullRender && newStart === _mpVirtWindowStart && newEnd === _mpVirtWindowEnd) return;

    var prevStart = _mpVirtWindowStart;

    if (forceFullRender || prevStart === -1) {
      var currentCards = [];
      try {
        currentCards = Array.prototype.slice.call(body.querySelectorAll(':scope > .post-card'));
      } catch (_e) {
        currentCards = Array.prototype.slice.call(body.querySelectorAll('.post-card'));
        currentCards = currentCards.filter(function(c) { return c.parentElement === body; });
      }
      currentCards.forEach(function(el) {
        try { body.removeChild(el); } catch (_e2) {}
      });

      var frag = document.createDocumentFragment();
      for (var i = newStart; i < newEnd; i++) {
        var card = _mpVirtCreateCard(_mpVirtPosts[i], _mpVirtData);
        card._mpVirtIdx = i;
        frag.appendChild(card);
      }

      if (_mpVirtGhostBelow && _mpVirtGhostBelow.parentElement === body) {
        body.insertBefore(frag, _mpVirtGhostBelow);
      } else {
      ...
      var existingCards;
      try {
        existingCards = Array.prototype.slice.call(body.querySelectorAll(':scope > .post-card'));
      } catch (_eQ) {
        existingCards = Array.prototype.slice.call(body.querySelectorAll('.post-card'));
        existingCards = existingCards.filter(function(c) { return c.parentElement === body; });
      }
      existingCards.forEach(function(el) {
        if (typeof el._mpVirtIdx === 'number') cardsByIdx[el._mpVirtIdx] = el;
      });
      ...
      for (var ri = 0; ri < idxKeys.length; ri++) {
        var rIdx = Number(idxKeys[ri]);
        if (rIdx < newStart || rIdx >= newEnd) {
          try { body.removeChild(cardsByIdx[rIdx]); } catch (_eRm) {}
          delete cardsByIdx[rIdx];
        }
      }
      ...
      for (var ai = newStart; ai < newEnd; ai++) {
        if (cardsByIdx[ai]) continue;
        var newCard = _mpVirtCreateCard(_mpVirtPosts[ai], _mpVirtData);
        newCard._mpVirtIdx = ai;
        var insertRef = null;
        for (var si = ai + 1; si < newEnd; si++) {
          if (cardsByIdx[si]) { insertRef = cardsByIdx[si]; break; }
        }
        if (insertRef) {
          body.insertBefore(newCard, insertRef);
        } else if (_mpVirtGhostBelow && _mpVirtGhostBelow.parentElement === body) {
          body.insertBefore(newCard, _mpVirtGhostBelow);
        } else {
          body.appendChild(newCard);
        }
        cardsByIdx[ai] = newCard;
      }
    }

    _mpVirtWindowStart = newStart;
    _mpVirtWindowEnd = newEnd;
    if (_mpVirtGhostAbove) _mpVirtGhostAbove.style.height = (newStart * gcH) + 'px';
    if (_mpVirtGhostBelow) _mpVirtGhostBelow.style.height = ((totalItems - newEnd) * gcH) + 'px';
  }

  function _mpVirtOnScroll() {
    if (_mpVirtRafPending) return;
    _mpVirtRafPending = true;
    requestAnimationFrame(function() {
      _mpVirtRafPending = false;
      _mpVirtRenderWindow(false);
    });
  }
  ...
  function renderPostList(posts) {
    ...
    // Clear existing list content (cards), preserving slack and ghost spacer elements.
    var _topS = postListEl.querySelector('.topSlack');
    var _botS = postListEl.querySelector('.bottomSlack');
    var _savedGhostAbove = postListEl.querySelector('.virt-ghost-above');
    var _savedGhostBelow = postListEl.querySelector('.virt-ghost-below');
    postListEl.innerHTML = '';
    if (_topS) postListEl.appendChild(_topS);
    if (_savedGhostAbove) _virtGhostAbove = _savedGhostAbove;
    if (_savedGhostBelow) _virtGhostBelow = _savedGhostBelow;
    ...
    // --- Virtualised scrolling ---
    _virtItems = _virtBuildItems(posts, _sfLookup, _sfGroups);
    _virtSortItems(currentSortKey, currentSortGeoLocation);

    // Handle preserved open post: find in sorted array, validate, prepare for window render
    _virtPreRendered = null;
    if (preservedOpenSlot && preservedOpenPostId) {
      for (var vi = 0; vi < _virtItems.length; vi++) {
        if (String(_virtItems[vi].post.id) !== preservedOpenPostId) continue;
        var _slotWasSF = !!preservedOpenSlot.querySelector('.post-storefront-menu-container');
        var _slotNowSF = !!_sfLookup[_virtItems[vi].post.id];
        var _sfGroupChanged = false;
        if (_slotWasSF && _slotNowSF) {
          var _oldIds = preservedOpenSlot.dataset.sfIds || '';
          var _newGroup = _sfGroups[_sfLookup[_virtItems[vi].post.id]] || [];
          var _newIds = _newGroup.map(function(p) { return String(p.id); }).join(',');
          if (_oldIds !== _newIds) _sfGroupChanged = true;
        }
        if (_slotWasSF !== _slotNowSF || _sfGroupChanged) break;
        ...
        try {
          ...
          if (storedList && _virtItems[vi].post.map_cards) {
            var freshById = {};
            _virtItems[vi].post.map_cards.forEach(function(mc2) { if (mc2 && mc2.id != null) freshById[mc2.id] = mc2; });
        ...
        _virtPreRendered = {};
        _virtPreRendered[vi] = _preservedOuter;
        break;
      }
    }

    // Ghost card spacer containers (created once, reused across re-renders)
    if (!_virtGhostAbove) {
      _virtGhostAbove = document.createElement('div');
      _virtGhostAbove.className = 'virt-ghost-above';
      _virtGhostAbove.style.overflow = 'hidden';
      BackdropComponent.populate(_virtGhostAbove);
    }
    if (!_virtGhostBelow) {
      _virtGhostBelow = document.createElement('div');
      _virtGhostBelow.className = 'virt-ghost-below';
      _virtGhostBelow.style.overflow = 'hidden';
      BackdropComponent.populate(_virtGhostBelow);
    }
    postListEl.appendChild(_virtGhostAbove);

    // Measure ghost card height once
    if (!_virtGhostCardHeight) {
      var _gcEl = _virtGhostAbove.querySelector('.post-backdrop-card');
      if (_gcEl) {
        var _gcStyle = window.getComputedStyle(_gcEl);
        _virtGhostCardHeight = (_gcEl.offsetHeight + parseFloat(_gcStyle.marginTop || 0) + parseFloat(_gcStyle.marginBottom || 0)) || 110;
      }
      if (!_virtGhostCardHeight) _virtGhostCardHeight = 110;
    }

    // Scroll to the preserved open post's position so the initial window includes it
    if (_virtPreRendered) {
      var _preIdx = Number(Object.keys(_virtPreRendered)[0]);
      if (_preIdx > 0 && _virtGhostCardHeight) {
        postListEl.scrollTop = _preIdx * _virtGhostCardHeight;
      }
    }

    // Render the initial window of ~50 elements
    _virtWindowStart = -1;
    _virtWindowEnd = -1;
    _virtRenderWindow(true);

    postListEl.appendChild(_virtGhostBelow);

    if (!_virtScrollBound) {
      postListEl.addEventListener('scroll', _virtOnScroll, { passive: true });
  ...
  function openMultipostModal(data) {
    ...
    // Collect matching post IDs in current sort order (from in-memory array)
    var orderedPostIds = [];
    (_virtItems || []).forEach(function(item) {
      var pid = String(item.post.id);
      if (postIds.indexOf(pid) !== -1) {
    ...
    if (orderedPosts.length) {
      _mpVirtPosts = orderedPosts;
      _mpVirtData = data;
      _mpVirtWindowStart = -1;
      _mpVirtWindowEnd = -1;

      _mpVirtGhostAbove = document.createElement('div');
      _mpVirtGhostAbove.className = 'virt-ghost-above';
      _mpVirtGhostAbove.style.overflow = 'hidden';
      BackdropComponent.populate(_mpVirtGhostAbove);

      _mpVirtGhostBelow = document.createElement('div');
      _mpVirtGhostBelow.className = 'virt-ghost-below';
      _mpVirtGhostBelow.style.overflow = 'hidden';
      BackdropComponent.populate(_mpVirtGhostBelow);

      body.appendChild(_mpVirtGhostAbove);
      _mpVirtRenderWindow(true);
      body.appendChild(_mpVirtGhostBelow);
      body.addEventListener('scroll', _mpVirtOnScroll, { passive: true });
    } else {
  ...
  function closeMultipostModal(options) {
    ...
    _mpVirtWindowStart = -1;
    _mpVirtWindowEnd = -1;
    _mpVirtGhostAbove = null;
    _mpVirtGhostBelow = null;
    _mpVirtRafPending = false;
  ...
  function _refreshMultipostModal() {
    ...
    // Collect posts at this location in current sort order (from in-memory array)
    var orderedPosts = [];
    (_virtItems || []).forEach(function(item) {
      var pid = String(item.post.id);
      if (_multipostModalPostIds.indexOf(pid) === -1) return;
      var post = (_lastRenderedPosts || []).filter(function(p) { return String(p.id) === pid; })[0];
      if (post) orderedPosts.push(post);
    });

    // Rebuild body with virtualised rendering
    var body = _multipostModalEl.querySelector('.multipost-modal-body');
    if (!body) return;
    body.innerHTML = '';
    ...
    if (orderedPosts.length) {
      ...
      if (!_mpVirtGhostAbove) {
        _mpVirtGhostAbove = document.createElement('div');
        _mpVirtGhostAbove.className = 'virt-ghost-above';
        _mpVirtGhostAbove.style.overflow = 'hidden';
        BackdropComponent.populate(_mpVirtGhostAbove);
      }
      if (!_mpVirtGhostBelow) {
        _mpVirtGhostBelow = document.createElement('div');
        _mpVirtGhostBelow.className = 'virt-ghost-below';
        _mpVirtGhostBelow.style.overflow = 'hidden';
        BackdropComponent.populate(_mpVirtGhostBelow);
  ...
  function sortPosts(sortKey, userGeoLocation) {
    if (!postListEl || !_virtItems || !_virtItems.length) return;
    _virtSortItems(sortKey, userGeoLocation);
    _virtRenderWindow(true);
