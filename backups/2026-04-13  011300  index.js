...
const App = (function() {
  ...
  function hasDeepLinkIntent() {
    try {
      ...
      if (qs.get('post') || qs.get('post_key') || qs.get('register') || qs.get('post-editor') || qs.get('post-editor-key') || qs.get('profile') || qs.has('privacy')) {
        return true;
      }
    } catch (_eQs) {}
    try {
      var path = String(window.location.pathname || '');
      return path.indexOf('/post/') !== -1 || path.indexOf('/register') !== -1 || path.indexOf('/post-editor') !== -1 || path.indexOf('/profile') !== -1 || path.indexOf('/privacy') !== -1;
    } catch (_ePath) {
      return false;
    }
  }
...
/* --------------------------------------------------------------------------
   ...
   Supported:
     ...
     /?privacy          or  /privacy              — opens Welcome modal → User Guide → Privacy Policy chapter
   -------------------------------------------------------------------------- */
document.addEventListener('DOMContentLoaded', function() {
  setTimeout(function() {
    try { if (window.PostModule) PostModule.handleDeepLink(); } catch (_e) {}

    // Privacy deep-link — opens welcome modal directly to Privacy Policy chapter
    try {
      var qs = new URLSearchParams(window.location.search || '');
      var path = String(window.location.pathname || '');
      if (qs.has('privacy') || path.indexOf('/privacy') !== -1) {
