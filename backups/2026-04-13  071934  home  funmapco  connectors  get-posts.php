...
try {
    ...
    if ($postId > 0 || $postKey !== '') {
        ...
        // ── Fetch priority system (single GROUP BY, uses idx_lat_lng) ────
        // One query groups by coordinate. Each row = one unique location with:
        //   - post_ids: all post IDs at that coordinate (GROUP_CONCAT)
        //   - cnt: number of distinct posts there
        //   - max_tier: highest checkout tier at that coordinate
        // PHP processes ~150 rows (unique coords), not 600+ (individual posts).
        //
        // Priority 1: Multi-post coordinates (cnt > 1) — uncapped
        // Priority 2: Solo premium (cnt = 1, max_tier >= 4) — uncapped
        // Priority 3: Solo featured (cnt = 1, max_tier = 3) — fills remaining slots
        // Priority 4: Solo standard (cnt = 1, max_tier <= 2) — fills remaining slots

        $coordLimit = $limit;

        $groupSql = "
            SELECT GROUP_CONCAT(DISTINCT p.id ORDER BY p.id) AS post_ids,
                   COUNT(DISTINCT p.id) AS cnt,
                   MAX(IFNULL(co.sort_order, 0)) AS max_tier
            FROM `posts` p
        ...
        ";
        $groupStmt = $mysqli->prepare($groupSql);
        if (!$groupStmt) fail(500, 'Failed to prepare group query: ' . $mysqli->error);
        bind_params_array($groupStmt, $types, $params);
        $groupStmt->execute();
        ...
        while ($groupRes && ($r = $groupRes->fetch_assoc())) {
            $cnt = (int)$r['cnt'];
            $maxTier = (int)$r['max_tier'];
            $ids = array_map('intval', explode(',', $r['post_ids']));

            if ($cnt > 1) {
                foreach ($ids as $pid) $guaranteedIds[$pid] = true;
                $guaranteedCoordCount++;
            } elseif ($maxTier >= 4) {
                $guaranteedIds[$ids[0]] = true;
                $guaranteedCoordCount++;
            } else {
                $soloCoords[] = ['pid' => $ids[0], 'tier' => $maxTier];
            }
        }
        $groupStmt->close();

        // Fill remaining coordinate slots: featured first, then standard.
        $remainingSlots = ($coordLimit > 0) ? max(0, $coordLimit - $guaranteedCoordCount) : PHP_INT_MAX;
        usort($soloCoords, function($a, $b) { return $b['tier'] - $a['tier']; });
        $slotsFilled = 0;
        foreach ($soloCoords as $sc) {
            if ($slotsFilled >= $remainingSlots) break;
            $guaranteedIds[$sc['pid']] = true;
            $slotsFilled++;
        }

        $pagePostIds = array_keys($guaranteedIds);
        $idsStmt = null;
    }
    ...
    // 2) Fetch posts + their map cards within the viewport
    ...
    $sql = "
        ...
        ($bounds !== null
            ? " AND mc.latitude BETWEEN ? AND ? AND mc.longitude BETWEEN ? AND ?"
            : "") .
    ...
    if ($bounds !== null) {
        $paramsBind2[] = $bounds['sw_lat'];
        $paramsBind2[] = $bounds['ne_lat'];
        $paramsBind2[] = $bounds['sw_lng'];
        $paramsBind2[] = $bounds['ne_lng'];
        $types2 .= 'dddd';
    }
