...
try {
    ...
    if ($postId > 0 || $postKey !== '') {
        ...
        // Priority 1: Multi-post coordinates (2+ posts at same lat/lng) — uncapped, all posts included
        ...
        // $limit caps the total number of unique coordinates for priorities 3+4 only.

        $coordLimit = $limit;

        // Step 1: Get all matching post IDs with their coordinate and tier info.
        $allSql = "
            SELECT p.id, mc.latitude, mc.longitude, IFNULL(co.sort_order, 0) AS tier_sort,
                   IFNULL(co.checkout_featured, 0) AS is_featured
            FROM `posts` p
            LEFT JOIN `post_map_cards` mc ON mc.post_id = p.id
            LEFT JOIN `checkout_options` co ON p.checkout_key = co.checkout_key
            WHERE {$whereClause}
        ...
        ";
        $allStmt = $mysqli->prepare($allSql);
        if (!$allStmt) fail(500, 'Failed to prepare priority query.');
        bind_params_array($allStmt, $types, $params);
        $allStmt->execute();
        $allRes = $allStmt->get_result();

        // Collect rows grouped by coordinate.
        $coordGroups = [];  // "lat,lng" => [post IDs]
        $postTier = [];     // post_id => tier_sort
        while ($allRes && ($r = $allRes->fetch_assoc())) {
            $pid = (int)$r['id'];
            $lat = $r['latitude'];
            $lng = $r['longitude'];
            $coordKey = ($lat !== null && $lng !== null) ? round((float)$lat, 6) . ',' . round((float)$lng, 6) : 'null_' . $pid;
            if (!isset($coordGroups[$coordKey])) $coordGroups[$coordKey] = [];
            if (!in_array($pid, $coordGroups[$coordKey])) $coordGroups[$coordKey][] = $pid;
            if (!isset($postTier[$pid])) $postTier[$pid] = (int)$r['tier_sort'];
        }
        $allStmt->close();

        // Step 2: Separate multi-post coords from solo coords.
        $guaranteedIds = [];     // Priority 1 + 2: always included
        $soloCoords = [];        // Priority 3 + 4: subject to coordinate cap
        $guaranteedCoordCount = 0;

        foreach ($coordGroups as $coordKey => $pids) {
            if (count($pids) > 1) {
                // Priority 1: multi-post coordinate — all posts guaranteed
                foreach ($pids as $pid) $guaranteedIds[$pid] = true;
                $guaranteedCoordCount++;
            } else {
                $pid = $pids[0];
                $tier = isset($postTier[$pid]) ? $postTier[$pid] : 0;
                // Premium = sort_order 4 (highest tier)
                $isPremium = ($tier >= 4);
                if ($isPremium) {
                    // Priority 2: solo premium — guaranteed
                    $guaranteedIds[$pid] = true;
                    $guaranteedCoordCount++;
                } else {
                    // Priority 3/4: solo featured/standard — subject to cap
                    $soloCoords[] = ['coordKey' => $coordKey, 'pid' => $pid, 'tier' => $tier];
                }
            }
        }

        // Step 3: Fill remaining coordinate slots with solo featured then standard.
        $remainingSlots = max(0, $coordLimit - $guaranteedCoordCount);
        usort($soloCoords, function($a, $b) { return $b['tier'] - $a['tier']; });
        $cappedIds = [];
        $slotsFilled = 0;
        foreach ($soloCoords as $sc) {
            if ($slotsFilled >= $remainingSlots) break;
            $cappedIds[$sc['pid']] = true;
            $slotsFilled++;
        }

        $pagePostIds = array_keys($guaranteedIds + $cappedIds);

        // Skip the standard fetch path below — we already have our IDs.
        $idsStmt = null;
    }
