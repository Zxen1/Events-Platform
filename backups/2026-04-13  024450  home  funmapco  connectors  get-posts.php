...
try {
    ...
    if ($postId > 0 || $postKey !== '') {
        ...
        // $limit cap commented out — all matching posts returned (markers are native Mapbox circles).
        // To re-enable: uncomment $limit line above, restore LIMIT/OFFSET below.
        $idsSql = "
            SELECT DISTINCT p.id
            FROM `posts` p
            LEFT JOIN `post_map_cards` mc ON mc.post_id = p.id
