...
### Post Editor Fly-To: Left Panel Behavior
**Problem**: When clicking an active postcard in the Post Editor, the map should fly to the location. During flight, the three LEFT panels (post, recent, filter) should close. On arrival, the post panel should reopen. The member panel (right side) must stay open throughout.

**Why it's stuck**: The header's mode switch buttons call `closePanels()` in `header.js` (line ~304), which closes ALL panels including the member panel. There's no way to switch modes (map → posts) without closing the member panel. The PostLocationComponent pattern (`mapBtn.click()` → fly → `postsBtn.click()`) can't be used directly.

**Current state**: `MapModule.flyTo(lng, lat)` is called. The map flies behind the member panel. No left panels close or reopen. This needs a proper solution — either modifying `closePanels()` to accept an exclusion, or finding another way to switch modes without closing the member panel.

**CRITICAL WARNING FOR NEXT AGENT:** Do NOT close the member panel under any circumstances during this flow. The user is editing their posts — closing the member panel to fly the map and then reopening it is not acceptable. Read `header.js` (specifically `closePanels()`), `PostLocationComponent`, and `MapModule.flyTo` thoroughly before implementing panel behavior. The previous agent broke this multiple times by acting before researching.
...
| `posteditor.js` | Status bar redesign (tier, layout, removed locations), postcard click handler (fly + toasts) |
...
### 1. Post Editor Fly-To — Left Panel Behavior (see INCOMPLETE above)
