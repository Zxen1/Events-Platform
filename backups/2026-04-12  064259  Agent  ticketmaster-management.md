...
1. Loads **ALL** imported attractions with active (non-expired) posts — `posts.expires_at > NOW()`
...
8. Stops when `max_api` API calls reached — tomorrow picks up where today stopped
9. Budget check is between attractions only — multi-page attractions are always completed

### Cycling behavior

The staleness-based ordering ensures every active post gets refreshed over time. There is no arbitrary LIMIT — the script loads all active attractions and works through them until the API budget is exhausted. Tomorrow it picks up from the least-recently refreshed attraction (which is the one after the last one processed today).
...
| **Map cards** | Full delete-and-reinsert (same as Post Editor). All map cards + children deleted, then reinserted from TM + saved past data |
| **Map card dates** | `start_date` and `end_date` recalculated from MIN/MAX of all `post_sessions` (past + future) after insert |
| **Sessions** | Past sessions saved before delete, reattached to new map cards by coordinate. Future sessions from TM API |
| **Pricing** | Past pricing saved and reattached. Future pricing from TM API |
| **Past-only venues** | Venues no longer in TM response are recreated from saved data with their past sessions. They expire naturally via `end_date` |
...
**Note:** Map card IDs change on every refresh (delete-and-reinsert). This is the same behavior as the Post Editor.
...
4. **Import** — Runs `tm-import.php` in a loop (up to 10 rounds) until no pending rows remain. No API calls.
...
| File | Changes |
|---|---|
| `tm-refresh.php` | Rewrote API loop: batches comma-separated attraction IDs per API call instead of one call per attraction. Queries `posts.tm_attraction_id` instead of GROUP BY on `tm_staging`. Tracks position via `refresh_position` in `_cursor` row JSON. Cycles back to start when all posts reached. Added transaction wrapping for delete-and-reinsert and zero-events blocks. |
| `tm-import.php` | Added `tm_attraction_id` to post INSERT during collation. |
| `edit-post.php` | Wrapped delete-and-reinsert in database transaction for crash safety. |
...
### ~~Rewrite Refresh: Delete-and-Reinsert, Remove Caps~~ ✓ DONE

Complete rewrite. Now uses delete-and-reinsert approach (same as Post Editor):
1. Save past sessions and pricing from database
2. Delete all map cards + children for the post
3. Reinsert map cards from TM response + reattach past sessions/pricing by coordinate matching
4. Past-only venues (no longer in TM) are recreated from saved data and expire naturally via `end_date`
...
**Schema change:** Added `tm_attraction_id` column (varchar 50, indexed) to `posts` table. Populated during collation (`tm-import.php`). NULL for non-TM posts.
