...
h. [TEMPLATE DONE] **707** `msg_email_post_deletion_warning` — Post deletion 7-day warning — fires at day 23 of the 30-day countdown. Cron trigger needed.
i. [TEMPLATE DONE] **708** `msg_email_post_deleted` — Post permanently deleted — fires when cron hard-deletes the post at day 30. Cron trigger needed.
...
m. [TEMPLATE DONE] **712** `msg_email_expiry_warning` — Expiry warning — daily cron job checks posts expiring within 14 days, sends once per member per cycle. Cron trigger needed.
n. [TEMPLATE DONE] **713** `msg_email_post_expired` — Post expired — same daily cron job, batched per member. Cron trigger needed.
...
r. [COMPLETE] **Unsubscribe link** — `{unsubscribe_link}` placeholder added to emails 712, 713, 714; `unsubscribed.html` page created; `email_notifications` column added to `members` and `admins`; profile form toggle (on/off with "(unsubscribed)" label when off); unsubscribed page POSTs to `edit-member.php` to set `email_notifications = 0`
...
**Templates in `admin_messages`** (container_key = `msg_email`, IDs 700–715):

| ID | message_key | Subject |
|----|-------------|---------|
| 700 | `msg_email_verification_code` | Your FunMap verification code |
| 701 | `msg_email_welcome` | Welcome to FunMap! |
| 702 | `msg_email_donation_thanks` | Your FunMap payment receipt |
| 703 | `msg_email_post_live` | Your listing is now live on FunMap |
| 704 | `msg_email_post_updated` | Your listing has been updated |
| 705 | `msg_email_password_reset` | Reset your FunMap password |
| 706 | `msg_email_post_deletion_requested` | Your FunMap listing is scheduled for deletion |
| 707 | `msg_email_post_deletion_warning` | Your FunMap listing will be deleted in 7 days |
| 708 | `msg_email_post_deleted` | Your FunMap listing has been deleted |
| 709 | `msg_email_account_deletion_requested` | Your account is scheduled for deletion |
| 710 | `msg_email_account_deletion_warning` | Your FunMap account will be deleted in 7 days |
| 711 | `msg_email_account_deleted` | Your FunMap account has been deleted |
| 712 | `msg_email_expiry_warning` | Your FunMap listings expire soon |
| 713 | `msg_email_post_expired` | Your FunMap listing has expired |
| 714 | `msg_email_inactivity_warning` | Your FunMap account will be closed in 30 days |
| 715 | `msg_email_inactivity_closed` | Your FunMap account has been closed |
