<?php
// cron-ticketmaster-daily.php — Full daily TM pipeline: collect → refresh → collate.
//
...
// Phase 2: Refresh — updates existing active posts via tm-refresh.php
// Phase 3: Collate — processes pending staging rows via tm-collate.php
// Phase 4: Clear   — NULLs event_json on processed rows to reclaim storage
//
// API budget: 5,000/day (1,000 reserve). Collect and refresh are reordered so
// all API calls land in a tight window, avoiding rolling-limit oscillation.
// Budget split is dynamic: seeding = 3k collect / 1k refresh;
// maintenance = 1k collect / 3k refresh.
//
...
$_logFile = $_logDir . '/tm-cron-' . date('Y-m-d-His') . '.log';
...
if ($seedingComplete) {
    ...
    $REFRESH_CEILING = $DAILY_BUDGET - $RESERVE - $COLLECT_CEILING;
} else {
    $COLLECT_CEILING = 3000;
    $REFRESH_CEILING = $DAILY_BUDGET - $RESERVE - $COLLECT_CEILING;
}

cronLog('COLLECT PHASE — pass ' . $pass . ', position ' . $position . '/' . $queueSize
    . ', ' . ($seedingComplete ? 'MAINTENANCE' : 'SEEDING')
    . " (collect={$COLLECT_CEILING}, refresh={$REFRESH_CEILING})");
...
while ($globalApiCalls < $COLLECT_CEILING) {
    ...
    if (preg_match('/\[RATE_LIMIT_AVAILABLE:(\d+)\]/', $output, $m)) {
        $lastRateLimitAvail = (int) $m[1];
    }
...
if ($lastRateLimitAvail !== null) {
    cronLog("TM budget remaining after collect (from API): {$lastRateLimitAvail}");
}
...
// ── Phase 2: Refresh existing posts ─────────────────────────────────────────
...
if ($refreshBudget < 100) {
    ...
    $_GET = ['max_api' => (string) $refreshBudget];
    $output = runPhase("{$dir}/tm-refresh.php");
    cronLogBlock($output);

    if (preg_match('/\[API_CALLS:(\d+)\]/', $output, $m)) {
        $refreshApiCalls = (int) $m[1];
        $globalApiCalls += $refreshApiCalls;
    }
    if (preg_match('/\[RATE_LIMIT_AVAILABLE:(\d+)\]/', $output, $m)) {
        cronLog("TM budget remaining (from API): {$m[1]}");
    }

    $_refreshTime = round(microtime(true) - $_refreshStart, 1);
    cronLog("Refresh done: {$refreshApiCalls} API calls, {$_refreshTime}s");
}

// ── Phase 3: Collate (staged rows into posts) ───────────────────────────────

cronLog('COLLATE PHASE');
$_collateStart = microtime(true);
$collateRound  = 0;
...
if ($pending === 0) {
    cronLog('No pending events to collate, skipping.');
} else {
    while ($pending > 0 && $collateRound < 10) {
        $collateRound++;
        cronLog("Collate round {$collateRound} ({$pending} pending)");

        $_GET = ['limit' => '200'];
        $output = runPhase("{$dir}/tm-collate.php");
        cronLogBlock($output);
...
$_collateTime = round(microtime(true) - $_collateStart, 1);
cronLog("Collate done: {$collateRound} round(s), {$_collateTime}s");

// ── Phase 4: Clear event_json from processed rows ───────────────────────────
