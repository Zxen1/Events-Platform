...
const PostModule = (function() {
  ...
  var _virtScrollCorrecting = false; // true while suppressing the scroll event caused by our own scrollTop correction
  ...
  function _virtShiftWindow(newStart, options) {
    ...
    // Set _virtScrollCorrecting first so the programmatic scrollTop change doesn't
    // re-trigger the scroll listener and cascade into another shift.
    if (!options || !options.noScrollCorrect) {
      var newAboveH = _virtAboveEl.offsetHeight;
      var _sDelta = newAboveH - oldAboveH;
      if (_sDelta !== 0) {
        _virtScrollCorrecting = true;
        postListEl.scrollTop = savedScroll + _sDelta;
      }
    }
  }
  ...
  function _virtOnScroll() {
    if (_virtScrollCorrecting) {
      _virtScrollCorrecting = false;
      return;
    }
