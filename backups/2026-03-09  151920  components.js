    ...
    function init(wrap, post, callbacks) {
        ...
        function bindSessionInteractions() {
            ...
            sessionOptionsPanel.addEventListener('mouseout', function(e) {
                if (!supportsHover()) return;
                // Don't hide if close timer is pending (user selected, waiting 500ms)
...
const StorefrontComponent = (function() {
    ...
    /**
     * Render the storefront section HTML.
     * @param {Object} options
     * @param {Array} options.items - Array of { thumbUrl, title } per post (pre-computed by post.js)
    ...
    function render(options) {
        ...
        items.forEach(function(item, i) {
            var emptyClass = item.thumbUrl ? '' : ' post-storefront-menu-item--empty';
            menuItems.push(
                '<span class="post-storefront-menu-item' + emptyClass + '" data-index="' + i + '" data-tooltip="' + escapeHtml(item.title) + '" data-tooltip-dir="right" tabindex="0" role="button" aria-pressed="false">' +
                    (item.thumbUrl
                        ? '<img class="post-storefront-menu-thumb" src="' + escapeHtml(item.thumbUrl) + '" alt="" referrerpolicy="no-referrer">'
                        : '<div class="post-storefront-menu-thumb"></div>') +
                '</span>'
            );
        });

        var html = [];
        html.push('<div class="post-storefront-menu" role="toolbar" aria-label="Storefront listings">' + menuItems.join('') + '</div>');
        html.push('<div class="post-storefront-prompt" data-message-key="msg_storefront_select_prompt"></div>');
        html.push('<div class="post-storefront-subheader post-storefront-subheader--hidden"></div>');
        html.push('<div class="post-storefront-content"></div>');
        return html.join('');
    }

    /**
     * Wire storefront behaviour.
     * @param {HTMLElement} wrap - The .post element
     * @param {Array} posts - The storefront posts array
     * @param {Object} callbacks
     * @param {Function} callbacks.onPostSelected - Called with (post, contentEl) when a post is clicked
     * @param {Function} callbacks.onAddToRecent - Called with (post) when a post is selected
     */
    function init(wrap, posts, callbacks) {
        var onPostSelected = (callbacks && callbacks.onPostSelected) || function() {};
        var onAddToRecent  = (callbacks && callbacks.onAddToRecent)  || function() {};
        ...
        // Load prompt message via getMessage (same pattern as PostSessionComponent)
        if (promptEl && typeof window.getMessage === 'function') {
            var promptKey = promptEl.getAttribute('data-message-key');
            if (promptKey) {
                window.getMessage(promptKey, {}, false).then(function(message) {
                    if (message && promptEl) promptEl.textContent = message;
                }).catch(function() {});
            }
        }

        // Set tooltip directions (60% width threshold — same pattern as setTooltipDirs in post.js)
        function updateTooltipDirs() {
            var containerRect = menuEl.getBoundingClientRect();
            if (!containerRect.width) return;
            menuEl.querySelectorAll('.post-storefront-menu-item[data-tooltip]').forEach(function(item) {
                var itemRect = item.getBoundingClientRect();
                item.setAttribute(
                    'data-tooltip-dir',
                    (itemRect.left - containerRect.left) > containerRect.width * 0.60 ? 'left' : 'right'
                );
            });
        }
        ...
        function selectPost(idx) {
            ...
            menuEl.querySelectorAll('.post-storefront-menu-item').forEach(function(item) {
                item.classList.remove('post-storefront-menu-item--selected');
                item.setAttribute('aria-pressed', 'false');
            });
            var selectedItem = menuEl.querySelector('.post-storefront-menu-item[data-index="' + idx + '"]');
            if (selectedItem) {
                selectedItem.classList.add('post-storefront-menu-item--selected');
                selectedItem.setAttribute('aria-pressed', 'true');
            }

            // Show subheader with selected post title
            if (subheaderEl) {
                var mc = post.map_cards && post.map_cards[0];
                var title = (mc && mc.title) || post.checkout_title || post.title || '';
                subheaderEl.textContent = title;
                subheaderEl.classList.remove('post-storefront-subheader--hidden');
            }

            // Hide prompt
            if (promptEl) {
                promptEl.classList.add('post-storefront-prompt--hidden');
            }

            // Callbacks
            onAddToRecent(post);
            onPostSelected(post, contentEl);
        }

        menuEl.addEventListener('click', function(e) {
            var item = e.target && e.target.closest ? e.target.closest('.post-storefront-menu-item') : null;
            if (!item) return;
