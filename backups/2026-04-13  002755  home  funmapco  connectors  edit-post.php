...
function abort_with_error(mysqli $mysqli, int $code, string $message, bool &$transactionActive): void
{
  ...
  if ($code >= 500) {
    ...
    $action = $GLOBALS['FUNMAP_GATEWAY_ACTION'] ?? 'edit-post';
...
function send_post_updated_email(mysqli $mysqli, int $member_id, string $member_role, string $to_name, int $post_id, ?int $transaction_id): void {
  ...
  // Fetch post title and post_key
  $pStmt = $mysqli->prepare("SELECT post_key FROM posts WHERE id = ? LIMIT 1");
  if (!$pStmt) return;
  $pStmt->bind_param('i', $post_id);
  $pStmt->execute();
  $pRow = $pStmt->get_result()->fetch_assoc();
  $pStmt->close();
  if (!$pRow) return;
  $postKey = $pRow['post_key'] ?? '';

  // Fetch post title from post_map_cards (primary card)
  $tStmt = $mysqli->prepare("SELECT title FROM post_map_cards WHERE post_id = ? ORDER BY id ASC LIMIT 1");
  $postTitle = '';
  ...
  $body      = str_replace(
    ...
    [$safeName, $safeTitle, htmlspecialchars($viewLink), htmlspecialchars($siteUrl . '/post-editor=' . $postKey), $logoHtml, $safeDesc, $amountHtml, (($siteSettings['website_prefix'] ?? '') ? $siteSettings['website_prefix'] . '-' : '') . str_pad($txId, 6, '0', STR_PAD_LEFT), $paymentVia, $dateStr],
...
// 2. Load existing map cards for comparison, then selectively rebuild (transaction)
$mysqli->begin_transaction();
try {

$existingCardsByCoord = [];
$mcResult = $mysqli->query("SELECT * FROM post_map_cards WHERE post_id = $postId");
if ($mcResult) {
  while ($mcRow = $mcResult->fetch_assoc()) {
    $coordKey = format_map_coord((float)$mcRow['latitude']) . '_' . format_map_coord((float)$mcRow['longitude']);
    $existingCardsByCoord[$coordKey] = $mcRow;
  }
  $mcResult->free();
}
...
foreach ($byLoc as $locNum => $entries) {
  $card = [
    ...
    'links_data' => null, 'ticket_url' => null, 'item_url' => null,
  ...
  $processedCoordKeys[$coordKey] = true;

  $existingCard = $existingCardsByCoord[$coordKey] ?? null;

  if ($existingCard) {
    // UPDATE existing card in place — preserves post_map_card_id
    $mapCardId = (int)$existingCard['id'];
    $timezone = null;
    $stmtUpdate = $mysqli->prepare(
      "UPDATE post_map_cards SET subcategory_key=?, title=?, description=?, media_ids=?,
       custom_text=?, custom_textarea=?, custom_dropdown=?, custom_checklist=?, custom_radio=?,
       public_email=?, phone_prefix=?, public_phone=?,
       location_type=?, venue_name=?, address_line=?, suburb=?, city=?, state=?, postcode=?,
       country_name=?, country_code=?, latitude=?, longitude=?, timezone=?,
       age_rating=?, ticket_url=?, item_url=?, price_summary=?, amenity_summary=?, updated_at=NOW()
       WHERE id=?"
    );
    $stmtUpdate->bind_param(
      'sssssssssssssssssssssddssssssi',
      ...
      $lat, $lng, $timezone,
      $card['age_rating'], $card['ticket_url'], $card['item_url'],
      $card['price_summary'], $card['amenity_summary'],
      $mapCardId
    );
    if (!$stmtUpdate->execute()) { $stmtUpdate->close(); abort_with_error($mysqli, 500, 'Update map card', $transactionActive); }
    $stmtUpdate->close();

    // Delete children — they'll be reinserted with correct data below
    $mysqli->query("DELETE FROM post_ticket_pricing WHERE post_map_card_id = $mapCardId");
    $mysqli->query("DELETE FROM post_item_pricing WHERE post_map_card_id = $mapCardId");
    $mysqli->query("DELETE FROM post_sessions WHERE post_map_card_id = $mapCardId");
    $mysqli->query("DELETE FROM post_amenities WHERE post_map_card_id = $mapCardId");
    if (table_exists($mysqli, 'post_links')) {
      $mysqli->query("DELETE FROM post_links WHERE post_map_card_id = $mapCardId");
    }
  } else {
    // INSERT new card
    $stmtCard = $mysqli->prepare("INSERT INTO post_map_cards (post_id, subcategory_key, title, description, media_ids, custom_text, custom_textarea, custom_dropdown, custom_checklist, custom_radio, public_email, phone_prefix, public_phone, location_type, venue_name, address_line, suburb, city, state, postcode, country_name, country_code, latitude, longitude, timezone, age_rating, ticket_url, item_url, price_summary, amenity_summary, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
    
    if (!$stmtCard) { abort_with_error($mysqli, 500, 'Prepare map card insert', $transactionActive); }
    $timezone = null;
  ...
  $venueInfoByCoord[$coordKey] = [
    ...
    if ($writeSessionPricing) {
      if (is_array($sessionsToWrite)) {
        ...
        $mysqli->query("UPDATE post_map_cards SET start_date = (SELECT MIN(ps.session_date) FROM post_sessions ps WHERE ps.post_map_card_id = {$mapCardId}), end_date = (SELECT MAX(ps.session_date) FROM post_sessions ps WHERE ps.post_map_card_id = {$mapCardId}) WHERE id = {$mapCardId}");
      }
...
// Delete map cards that no longer exist in submission
foreach ($existingCardsByCoord as $ck => $oldCard) {
  if (isset($processedCoordKeys[$ck])) continue;
  $rmId = (int)$oldCard['id'];
  $mysqli->query("DELETE FROM post_ticket_pricing WHERE post_map_card_id = $rmId");
  $mysqli->query("DELETE FROM post_item_pricing WHERE post_map_card_id = $rmId");
  $mysqli->query("DELETE FROM post_sessions WHERE post_map_card_id = $rmId");
  $mysqli->query("DELETE FROM post_amenities WHERE post_map_card_id = $rmId");
  if (table_exists($mysqli, 'post_links')) {
    $mysqli->query("DELETE FROM post_links WHERE post_map_card_id = $rmId");
  }
  $mysqli->query("DELETE FROM post_map_cards WHERE id = $rmId");
}
...
$mysqli->commit();
  ...
  $mysqli->rollback();
  echo json_encode(['success' => false, 'message' => 'Save failed, no data was changed: ' . $e->getMessage()]);
  exit;
}
...
$mysqli->query("UPDATE post_map_cards SET start_date = COALESCE(start_date, (SELECT DATE(created_at) FROM posts WHERE id = {$postId})), end_date = (SELECT DATE(expires_at) FROM posts WHERE id = {$postId}) WHERE post_id = {$postId} AND end_date IS NULL");
