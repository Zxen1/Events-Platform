...
foreach ($byLoc as $locNum => $entries) {
  ...
  if (is_array($card['links_data']) && count($card['links_data']) > 0) {
    if (table_exists($mysqli, 'post_links')) {
      $stmtLinks = $mysqli->prepare("INSERT INTO post_links (post_map_card_id, link_type, external_url, is_active, created_at, updated_at)
        VALUES (?, ?, ?, 1, NOW(), NOW())");
      if ($stmtLinks) {
        $linkCount = 0;
...
if ($freeCoupon && $couponId > 0) {
    ...
    $newTxStmt = $mysqli->prepare(
        "INSERT INTO transactions (member_id, post_id, transaction_type, member_role, checkout_key, payment_id, payment_gateway, payment_method, quote, discount, coupon_id, total, currency, line_items, description, status, created_at, updated_at)
         VALUES (?, ?, 'new_post', ?, ?, NULL, 'free', 'coupon', ?, ?, ?, 0, ?, NULL, ?, 'paid', NOW(), NOW())"
    );
    if ($newTxStmt) {
