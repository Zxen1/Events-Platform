...
## Code Locations (all in `post.js`)

| Function | Line | What Changes |
|----------|------|--------------|
| `renderPostList(posts)` | 1798 | **Primary target.** Currently loops through all posts creating a DOM element for each. Change to: create ~50 elements, set up scroll listener for recycling, fill remaining space with ghost postcards. |
| `renderPostCard(post, options)` | 1425 | No logic changes, but will be called by the recycler to fill/refill elements. May need to support "refill existing element" in addition to "create new element." |
| `renderStorefrontCard(sfPosts)` | 1615 | Same as above — may need a refill mode for recycling. |
| `openMultipostModal(data)` | 2701 | Change DOM queries (lines 2718-2725 collect `.post-card` elements from DOM) to read from in-memory array. Add virtualised scrolling for the modal's own postcard list. |
| `_refreshMultipostModal()` | 2893 | Same — reads from DOM, needs to read from in-memory array. |
| `closeMultipostModal(options)` | 2869 | No changes expected. |

### Key variables to be aware of:
- `_lastRenderedPosts` — the in-memory array of all posts from the last fetch. This is the data source for the virtualised window.
- `_sfGroups` / `_sfGroupsByPostId` / `_sfLookup` — storefront grouping data, built in `renderPostList` before the DOM loop. This logic stays, only the DOM loop changes.
- `postListEl` — the scroll container that holds postcards. The scroll listener attaches here.
- `_multipostModalPostIds` — stored post IDs for the multipost modal. Modal virtualisation reads from this + `_lastRenderedPosts`.
