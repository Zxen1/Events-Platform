 ...
 * already have active (non-expired) posts. Loads ALL active attractions,
 * ordered by least-recently refreshed, and processes them until the API
 * budget is exhausted. Tomorrow picks up where today stopped.
 *
 * Budget check is between attractions only — a multi-page attraction is
 * always completed even if that exceeds the budget slightly.
 *
...
echo "Ticketmaster refresh — " . count($attractions) . " attractions queued (budget: {$maxApi})\n";
echo str_repeat('─', 72) . "\n\n";
...
foreach ($attractions as $attractionId => $postId) {
    ...
    // ── Save past data, delete, reinsert (transaction) ─────────────────────────
    ...
    while ($card = $cardRes->fetch_assoc()) {
        ...
        if (!empty($pastSessions)) {
            $pastData[$ck] = ['sessions' => $pastSessions, 'pricing' => $pastPricing];
        }
    }

    // ── Delete all map cards + children for this post ──────────────────────────

    $oldCardIds = [];
    foreach ($savedCards as $card) $oldCardIds[] = (int) $card['id'];

    if (!empty($oldCardIds)) {
        $oldIdList = implode(',', $oldCardIds);
        $mysqli->query("DELETE FROM post_ticket_pricing WHERE post_map_card_id IN ({$oldIdList})");
        $mysqli->query("DELETE FROM post_item_pricing WHERE post_map_card_id IN ({$oldIdList})");
        $mysqli->query("DELETE FROM post_sessions WHERE post_map_card_id IN ({$oldIdList})");
        $mysqli->query("DELETE FROM post_amenities WHERE post_map_card_id IN ({$oldIdList})");
    }
    $mysqli->query("DELETE FROM post_map_cards WHERE post_id = {$postId}");

    // ── Reinsert map cards from TM + past data ────────────────────────────────
    ...
    foreach ($validVenueGroups as $vk => $vEvts) {
        ...
        // Reinsert past sessions for this venue
        $pastCount = 0;
        if (isset($pastData[$ck])) {
            foreach ($pastData[$ck]['sessions'] as $ps) {
                $stmt = $mysqli->prepare(
            ...
            foreach ($pastData[$ck]['pricing'] as $pp) {
        ...
        // Recalculate start_date/end_date from all sessions
        $mysqli->query(
            "UPDATE post_map_cards SET
    ...
    // ── Reinsert past-only venues (no longer in TM response) ───────────────────

    foreach ($pastData as $ck => $pd) {
        if (isset($processedCoords[$ck])) continue;

        $oldCard = $savedCards[$ck];
        $oldLat  = (float) $oldCard['latitude'];
        $oldLng  = (float) $oldCard['longitude'];
        ...
        $oldLocType    = $oldCard['location_type'] ?? 'venue';
        $oldMediaIds   = $oldCard['media_ids'] ?? '';
        $oldVenue      = $oldCard['venue_name'] ?? '';
        $oldAddr       = $oldCard['address_line'] ?? '';
        $oldCity       = $oldCard['city'] ?? '';
        $oldState      = $oldCard['state'];
        $oldPostcode   = $oldCard['postcode'];
        $oldCountry    = $oldCard['country_name'] ?? '';
        $oldCC         = $oldCard['country_code'] ?? '';
        $oldTz         = $oldCard['timezone'] ?? 'UTC';
        $oldTicket     = $oldCard['ticket_url'] ?? '';
        $oldPrice      = $oldCard['price_summary'];
        $oldSubcat     = $oldCard['subcategory_key'] ?? $freshSubcat;
        $oldTitle      = $oldCard['title'] ?? $freshTitle;
        $oldDesc       = $oldCard['description'] ?? '';

        $stmt->bind_param(
            'issssssssssssddsss',
            $postId, $oldSubcat, $oldTitle, $oldDesc, $oldMediaIds,
            $oldLocType, $oldVenue, $oldAddr, $oldCity, $oldState,
            $oldPostcode, $oldCountry, $oldCC,
            $oldLat, $oldLng, $oldTz,
            $oldTicket, $oldPrice
        );
        $stmt->execute();
        $pastCardId = $mysqli->insert_id;
        $stmt->close();

        foreach ($pd['sessions'] as $ps) {
            $stmt = $mysqli->prepare(
                "INSERT INTO `post_sessions` (`post_map_card_id`,`session_date`,`session_time`,`ticket_group_key`)
                 VALUES (?,?,?,?)"
            );
            $stmt->bind_param('isss', $pastCardId, $ps['session_date'], $ps['session_time'], $ps['ticket_group_key']);
            $stmt->execute();
            $stmt->close();
        }
        foreach ($pd['pricing'] as $pp) {
            $ppAllocated = (int) $pp['allocated_areas'];
            $ppPrice     = (float) $pp['price'];
            $stmt = $mysqli->prepare(
                "INSERT INTO `post_ticket_pricing`
            ...
            $stmt->bind_param('isissds', $pastCardId, $pp['ticket_group_key'], $ppAllocated, $pp['ticket_area'], $pp['pricing_tier'], $ppPrice, $pp['currency']);
            $stmt->execute();
            $stmt->close();
        }
        ...
        $mysqli->query(
            "UPDATE post_map_cards SET
             start_date = (SELECT MIN(ps.session_date) FROM post_sessions ps WHERE ps.post_map_card_id = {$pastCardId}),
             end_date = (SELECT MAX(ps.session_date) FROM post_sessions ps WHERE ps.post_map_card_id = {$pastCardId})
             WHERE id = {$pastCardId}"
        );
    }
