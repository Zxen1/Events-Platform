...
g. **706** `msg_email_post_deletion_requested` — Post scheduled for deletion — fires when member marks a post for deletion; warns it will be permanently removed in 30 days
...
j. **709** `msg_email_account_deletion_requested` — Account scheduled for deletion — fires when member requests account deletion; 30-day grace period (template exists, trigger needed in `delete-member.php`)
...
r. **Unsubscribe link** — legally required in many countries; add one-click unsubscribe to all emails
