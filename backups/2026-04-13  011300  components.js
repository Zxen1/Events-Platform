...
const WelcomeModalComponent = (function() {
    ...
    function loadGuide(expandChapterKey) {
        ...
        fetch('/gateway.php?action=get-admin-settings&lite=1&include_user_guide=true')
            ...
            .then(function(data) {
                if (data.success && data.user_guide && data.user_guide.length) {
                    renderUserGuideChapters(userGuideElement.querySelector('.welcome-user-guide-chapters'), data.user_guide, expandChapterKey);
                }
            })
            .catch(function(err) {
    ...
    function renderUserGuideChapters(chaptersContainer, items, expandChapterKey) {
        ...
        items.forEach(function(item) {
            if (!chapterMap[item.chapter]) {
                chapterMap[item.chapter] = { chapter: item.chapter, chapter_key: item.chapter_key || null, items: [] };
        ...
        chapters.forEach(function(chapterData) {
            ...
            if (chapterData.chapter_key) {
                chapter.setAttribute('data-chapter-key', chapterData.chapter_key);
            }
            ...
            var shouldExpand = expandChapterKey && chapterData.chapter_key === expandChapterKey;

            var chapterBody = document.createElement('div');
            chapterBody.className = 'welcome-user-guide-chapter-body' + (shouldExpand ? '' : ' welcome-user-guide-chapter-body--hidden');
            ...
            if (shouldExpand) {
                chapterArrow.classList.add('welcome-user-guide-chapter-arrow--open');
            }
    ...
    function openToChapter(chapterKey) {
        if (!modal) init();
        open();
        loadGuide(chapterKey);
    }
...
const PostLocationComponent = (function() {
     ...
     */
    var TM_MEMBER_ID = 213;

    function buildLocationMain(loc) {
        var locationType = loc.location_type || '';
        if (locationType === 'city') return loc.city || '';
        if (locationType === 'address') return loc.address_line || '';
        return loc.venue_name || '';
    }

    function buildLocationSecondary(loc, memberId) {
        var addressLine = loc.address_line || '';
        var city = loc.city || '';
        var state = loc.state || '';
        var countryName = loc.country_name || '';
        var locationType = loc.location_type || '';
        var isTm = memberId === TM_MEMBER_ID;
    ...
    function renderLocationOption(loc, index, isSelected, escapeHtml, isFiltered, memberId) {
        var mainText = buildLocationMain(loc);

        var cls = 'post-location-option';
        if (isSelected) cls += ' post-location-highlighted';
        if (isFiltered) cls += ' post-location-option--filtered';

        var secondary = buildLocationSecondary(loc, memberId);

        var html = [];
        html.push('<div class="' + cls + '" data-index="' + index + '">');
        html.push('<div class="post-location-option-main">' + escapeHtml(mainText) + '</div>');
        if (secondary) {
    ...
    function render(options) {
        var postId = options.postId || '';
        var memberId = options.memberId || 0;
        ...
        var mainText = buildLocationMain(loc0);
        var secondary = buildLocationSecondary(loc0, memberId);

        var html = [];

        html.push('<div class="post-location-container" data-post-id="' + postId + '" data-member-id="' + memberId + '">');
        ...
        html.push('<div class="post-location-text-main">' + escapeHtml(mainText) + '</div>');
        if (secondary) {
        ...
        for (var i = 0; i < locationList.length; i++) {
            ...
            html.push(renderLocationOption(locationList[i], i, i === 0, escapeHtml, filtered, memberId));
        }
        html.push('</div>');
    ...
    function init(wrap, post, callbacks) {
        ...
        locationOptions.forEach(function(opt) {
            opt.addEventListener('click', function(e) {
                ...
                if (btnTextMain) {
                    btnTextMain.textContent = buildLocationMain(loc);
                }
                if (btnTextSecondary) {
                    btnTextSecondary.textContent = buildLocationSecondary(loc, post.member_id);
                }
