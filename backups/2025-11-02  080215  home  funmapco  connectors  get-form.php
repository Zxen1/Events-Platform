...
function sanitizeFieldOptions(string $type, $options): array
{
    ...
    $iterable = normalizeFieldOptionIterable($options);

    if ($type === 'variant_pricing') {
        $normalized = [];
        foreach ($iterable as $option) {
            if (is_array($option)) {
                $version = isset($option['version']) && is_string($option['version'])
                    ? trim($option['version'])
                    : '';
                $currency = isset($option['currency']) && is_string($option['currency'])
                    ? trim($option['currency'])
                    : '';
                $price = isset($option['price']) && is_string($option['price'])
                    ? trim($option['price'])
                    : '';

                if ($version !== '' || $currency !== '' || $price !== '') {
                    $normalized[] = [
                        'version' => $version,
                        'currency' => $currency,
                        'price' => $price,
                ...
                $strings = collectFieldOptionStrings($option);
...
function normalizeFieldOptionIterable($options): array
{
    if (!is_array($options)) {
        return [];
    }
    ...
    while ($queue && $iterations < 1000) {
        ...
        $foundWrapper = false;
        foreach ($wrapperKeys as $wrapperKey) {
            if (array_key_exists($wrapperKey, $current) && is_array($current[$wrapperKey])) {
                $queue[] = $current[$wrapperKey];
                $foundWrapper = true;
            }
        }
        ...
        if ($foundWrapper) {
            foreach ($current as $key => $value) {
                if (in_array($key, $wrapperKeys, true)) {
                    continue;
                }
...
function collectFieldOptionStrings(array $value): array
{
    $strings = [];
    $stack = [$value];
