...
(function(){
  ...
  const persistedFormbuilderSnapshotPromise = (()=>{
    if(typeof window !== 'undefined'){
      const existing = window.persistedFormbuilderSnapshotPromise;
      if(existing && typeof existing.then === 'function'){
        return existing;
      }
      const cached = window.__persistedFormbuilderSnapshotPromise;
    ...
    if(inlineSnapshot){
      const promise = Promise.resolve(inlineSnapshot);
      if(typeof window !== 'undefined'){
        window.__persistedFormbuilderSnapshotPromise = promise;
      }
      return promise;
    }
    const promise = fetchSavedFormbuilderSnapshot();
    if(typeof window !== 'undefined'){
      window.__persistedFormbuilderSnapshotPromise = promise;
    }
    return promise;
...
(function(){
  ...
  function normalizeIconLibraryEntriesLocal(entries){
    const seen = new Set();
    const normalized = [];
    if(!Array.isArray(entries)){
      return normalized;
  ...
  function normalizeIconAssetPathLocal(path){
    const normalized = baseNormalizeIconPath(path);
    if(!normalized){
      return '';
    }
  ...
  if(typeof window !== 'undefined'){
    window.normalizeIconPath = (path)=>{
      ...
      return normalizeIconAssetPathLocal(initial);
    };
  }

  function normalizeIconPathMapLocal(source){
    const normalized = {};
    if(!source || typeof source !== 'object'){
      return normalized;
    }
    Object.keys(source).forEach(key => {
      const rawValue = source[key];
      const value = typeof rawValue === 'string' ? normalizeIconAssetPathLocal(rawValue) : '';
      if(typeof key !== 'string'){
        return;
      }
      const trimmed = key.trim();
  ...
  const MULTI_POST_MARKER_ICON_ID = typeof window.MULTI_POST_MARKER_ICON_ID === 'string'
  ...
  window.getFormFieldTypeLabel = getFormFieldTypeLabel;
  window.__getVenueAutofillState = getVenueAutofillState;
  window.__resetVenueAutofillState = resetVenueAutofillState;
  window.normalizeIconPathMap = normalizeIconPathMapLocal;
  window.normalizeIconAssetPath = normalizeIconAssetPathLocal;
  window.normalizeIconLibraryEntries = normalizeIconLibraryEntriesLocal;
  window.lookupIconPath = lookupIconPath;
  window.writeIconPath = writeIconPath;
  window.renameIconNameKey = renameIconNameKey;
  window.deleteIconKeys = deleteIconKeys;
  window.closeAllIconPickers = closeAllIconPickers;
  window.baseNormalizeIconPath = baseNormalizeIconPath;
  window.applyNormalizeIconPath = applyNormalizeIconPath;
  window.getCategoryIconPath = getCategoryIconPath;
  window.getSubcategoryIconPath = getSubcategoryIconPath;
  window.extractIconSrc = extractIconSrc;
})();
...
(function(){
  ...
  const subcategoryMarkerIds = window.subcategoryMarkerIds = window.subcategoryMarkerIds || {};
  const categoryShapes = window.categoryShapes = window.categoryShapes || {};
  const categoryControllers = window.categoryControllers || {};
  const allSubcategoryKeys = window.allSubcategoryKeys || [];
  const selection = window.categorySelection || { cats: new Set(), subs: new Set() };
  const slugify = typeof window.slugify === 'function' ? window.slugify : (str => (str || "").toString().trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, ''));
  const renderFilterCategories = typeof window.renderFilterCategories === 'function'
    ? window.renderFilterCategories
    : ()=>{};
  const normalizeFormbuilderSnapshot = window.normalizeFormbuilderSnapshot || (value => value || {});
  const getSavedFormbuilderSnapshot = window.getSavedFormbuilderSnapshot || (()=>null);
  const normalizeIconPathMap = window.normalizeIconPathMap || (value => value || {});
  const normalizeIconAssetPath = window.normalizeIconAssetPath || (value => value || "");
  const persistedFormbuilderSnapshotPromise = window.persistedFormbuilderSnapshotPromise || Promise.resolve(null);
  const ICON_LIBRARY = Array.isArray(window.iconLibrary) ? window.iconLibrary : [];
  const FORM_FIELD_TYPES = Array.isArray(window.FORM_FIELD_TYPES) ? window.FORM_FIELD_TYPES : [];
  const OPEN_ICON_PICKERS = window.__openIconPickers || new Set();
  window.__openIconPickers = OPEN_ICON_PICKERS;
