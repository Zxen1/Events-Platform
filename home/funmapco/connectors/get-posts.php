...
try {
    ...
    // For single-post loads (post_id / post_key) these only affect passes_filter
    // so the post always loads but the location menu can show which locations match.
    ...
    // For list loads, user-facing filters restrict results
    if (!$isSinglePostLoad) {
    ...
    // Snapshot for passes_filter (Step 1b): base + user-facing filters, no bounds.
