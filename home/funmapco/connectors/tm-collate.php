    ...
    $stmt  = $mysqli->prepare(
        "INSERT INTO `posts`
         (`post_key`,`member_id`,`member_role`,`member_name`,`subcategory_key`,
          ...
          `checkout_key`,`days_purchased`,`payment_status`,`tm_attraction_id`,`expires_at`)
         VALUES ('TEMP',?,?,?,?,?,?,'active','clean','premium',0,'paid',?,?)"
    );
    $stmt->bind_param('isssiiss', $mid, $mrole, $mname, $subcatKey, $locQty, $locQty, $attractionId, $expiresAt);
    $stmt->execute();
