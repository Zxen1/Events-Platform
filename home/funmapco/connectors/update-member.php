...
if (empty($data['id'])) exit(json_encode(['error'=>'Missing member ID']));

$stmt = $mysqli->prepare("UPDATE members SET display_name=?, avatar_url=?, theme=?, language=?, currency=? WHERE id=?");
$stmt->bind_param('sssssi', $data['display_name'], $data['avatar_url'], $data['theme'], $data['language'], $data['currency'], $data['id']);
$stmt->execute();
