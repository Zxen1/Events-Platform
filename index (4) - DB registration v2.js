...
// === Registration DB Hook (non-visual, safe override) ===
(function(){
  const registerPanel = document.getElementById('memberRegisterPanel');
  if(!registerPanel) return;

  // Ensure confirm field exists, but don't restyle.
  ...
  // Capture all submit events within register panel and override only for register action.
  registerPanel.addEventListener('submit', async function(e){
    ...
    if(action !== 'register') return; // let non-register submits proceed as-is

    // Always prevent default old localStorage path.
    e.preventDefault();
    e.stopImmediatePropagation();

    const displayNameEl = registerPanel.querySelector('input[name=\"display_name\"], #memberRegisterName, input[placeholder*=\"Name\" i]');
    const emailEl = registerPanel.querySelector('input[type=\"email\"], input[name=\"email\"], #memberRegisterEmail');
    const passEl = pwd1 || registerPanel.querySelector('input[type=\"password\"]');
    const confirmEl = pwd2 || registerPanel.querySelector('#memberRegisterPasswordConfirm');
    ...
    if(!display_name || !email || !password){
      (window.showStatus ? window.showStatus('Please fill all required fields.', {error:true}) : alert('Please fill all required fields.'));
      return;
    }
    if(password !== confirm){
      (window.showStatus ? window.showStatus('Passwords do not match.', {error:true}) : alert('Passwords do not match.'));
      if(confirmEl && confirmEl.focus) confirmEl.focus();
      return;
    }
    ...
    try{
      ...
      const text = await res.text();
      let data = null;
      try{ data = JSON.parse(text); }catch{ data = { error: 'Invalid server response', raw: text }; }

      if(data && data.success){
        // Only here may we trigger any local "logged-in" state the site uses.
        (window.showStatus ? window.showStatus('Account created successfully.') : console.log('Account created successfully.'));
        // Optionally reload or switch to login tab if your UI requires that; we avoid visual changes per rules.
      } else {
        const msg = (data && data.error) ? data.error : 'Registration failed.';
        (window.showStatus ? window.showStatus(msg, {error:true}) : alert(msg));
      }
    } catch(err){
      (window.showStatus ? window.showStatus('Server error during registration.', {error:true}) : alert('Server error during registration.'));
      console.error(err);
    }
  }, { capture: true });
})();
// === End Registration DB Hook ===
