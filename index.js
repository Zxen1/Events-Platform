...
// === Strict Registration Override (DB-only, no localStorage auto-login) ===
(function(){
  const panel = document.getElementById('memberRegisterPanel');
  if(!panel) return;

  // Ensure confirm field (non-visual, reuse existing styles)
  const pass1 = panel.querySelector('input[type="password"]');
  let pass2 = panel.querySelector('#memberRegisterPasswordConfirm');
  if(pass1 && !pass2){
    pass2 = document.createElement('input');
    pass2.type = 'password';
    pass2.id = 'memberRegisterPasswordConfirm';
    pass2.placeholder = 'Confirm Password';
    pass2.autocomplete = 'new-password';
    pass2.required = true;
    if(pass1.className) pass2.className = pass1.className;
    pass1.insertAdjacentElement('afterend', pass2);
  }

  // Find form that contains the register panel
  let form = panel.closest('form');
  if(!form) form = document.getElementById('memberForm');

  function status(msg, error){
    if(typeof showStatus === 'function'){ showStatus(msg, {error: !!error}); }
    else if(error){ console.error(msg); } else { console.log(msg); }
  }

  if(form){
    form.addEventListener('submit', async (e) => {
      const submitter = e.submitter || document.activeElement;
      const action = submitter && submitter.dataset && submitter.dataset.action ? submitter.dataset.action : 'register';
      if(action !== 'register') return; // Let non-register submits pass

      // Block any previous handler from running (prevents localStorage path)
      e.preventDefault();
      e.stopImmediatePropagation();

      const nameEl   = panel.querySelector('input[name="display_name"], #memberRegisterName, input[placeholder*="Name" i]');
      const emailEl  = panel.querySelector('input[type="email"], input[name="email"], #memberRegisterEmail');
      const avatarEl = panel.querySelector('input[name="avatar_url"], #memberRegisterAvatar');

      const display_name = (nameEl?.value || '').trim();
      const email = (emailEl?.value || '').trim();
      const password = pass1 ? pass1.value : '';
      const confirm  = pass2 ? pass2.value : '';
      const avatar_url = (avatarEl?.value || '').trim();

      if(!display_name || !email || !password){
        status('Please fill all required fields.', true); return;
      }
      if(password !== confirm){
        if(!window.__pwErrorShown){window.__pwErrorShown=true;status('Passwords do not match.', true);setTimeout(()=>window.__pwErrorShown=false,500);}
        try{ pass2 && pass2.focus(); }catch(_){}
        return;
      }

      try{
        const res = await fetch('connectors/add-member.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ display_name, email, password, confirm, avatar_url })
        });
        const txt = await res.text();
        let data; try{ data = JSON.parse(txt); }catch{ data = { success:false, error:'Invalid server response', raw: txt }; }

        if(data && data.success){
          status('Account created successfully.');
          // Do NOT auto-login here; require user to login separately.
        } else {
          status(data && data.error ? data.error : 'Registration failed.', true);
        }
      } catch(err){
        console.error(err);
        status('Server error during registration.', true);
      }
    }, { capture: true }); // capture to beat older bubbling handlers
  }
})();
// === End Strict Registration Override ===
