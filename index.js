...
// === Admin login verifier (calls secure server-side auth in /config/admin-auth.php) ===
async function verifyAdminLogin(username, password){
  try{
    const res = await fetch('/config/admin-auth.php', {
      method: 'POST',
...
const adminAuthManager = (()=>{
  ...
  return {
    ...
    matchesCredentials(identifier, password){
      return normalizeIdentifier(identifier) === 'admin' && password === '1234';
    },
    isAdminIdentifier(identifier){
      return normalizeIdentifier(identifier) === 'admin';
    },
    getAdminUser(){
