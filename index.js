...
function mulberry32(a){ return function(){var t=a+=0x6D2B79F5; t=Math.imul(t^t>>>15, t|1); t^=t+Math.imul(t^t>>>7, t|61); return ((t^t>>>14)>>>0)/4294967296; }; }
    ...
    const cities = [
      ...
      {n:"ReykjavÃ­k, Iceland", c:[-21.8174,64.1265]},
  ...
  function addVenueToLocalIndex({ name, address, lng, lat, city }){
    ...
    const placeName = contextParts.length ? `${name} â€” ${contextParts.join(', ')}` : name;
...
function makePosts(){
  ...
  const hubs = [
    ...
    {c:"ReykjavÃ­k, Iceland", lng:-21.8277, lat:64.1265},
    {c:"Moscow, Russia",     lng:37.6173, lat:55.7558},
    {c:"Istanbul, TÃ¼rkiye",  lng:28.9784, lat:41.0082},
    ...
    {c:"SÃ£o Paulo, Brazil",  lng:-46.6333, lat:-23.5505},
  ...
  const singleVenueBases = [
    ...
    { city: "BogotÃ¡, Colombia", lng: -74.0721, lat: 4.7110 },
    ...
    { city: "SÃ£o Paulo, Brazil", lng: -46.6333, lat: -23.5505 },
    { city: "Rio de Janeiro, Brazil", lng: -43.1729, lat: -22.9068 },
    { city: "BrasÃ­lia, Brazil", lng: -47.8825, lat: -15.7942 },
    ...
    { city: "ReykjavÃ­k, Iceland", lng: -21.8277, lat: 64.1265 },
    ...
    { city: "KrakÃ³w, Poland", lng: 19.9440, lat: 50.0647 },
    ...
    { city: "Istanbul, TÃ¼rkiye", lng: 28.9784, lat: 41.0082 },
    { city: "Ankara, TÃ¼rkiye", lng: 32.8597, lat: 39.9334 },
    ...
    { city: "Abidjan, CÃ´te d'Ivoire", lng: -4.0083, lat: 5.3599 },
  ...
  function assignMultiPosts(postList, targetCount){
    ...
    for(let idx = 0; idx < indices.length && assigned < targetCount; idx++){
      ...
      const nextLocations = venues.map((venue, venueIdx) => {
        ...
        const venueLabel = `${cityLabel} Â· Spot ${venueIdx + 1}`;
        return {
          venue: venueLabel,
          address: cityLabel,
          lng: venue.lng,
  ...
  return out;
      ...
      async function openPost(id, fromHistory=false, fromMap=false, originEl=null){
        ...
        const scrollToTop = (attempt = 1) => {
          ...
          if(scrollElement && typeof scrollElement.scrollTop !== 'undefined'){
            ...
            console.log(
              `[openPost] Scroll attempt ${attempt} (${entryPoint}): ${scrollSuccess ? 'âœ“ SUCCESS' : 'âœ— FAILED'}`,
              ...
              `\n  Before: ${beforeScroll}px â†’ After: ${afterScroll}px`,
            ...
            if(!scrollSuccess && attempt === 3){
              console.error(
                `[openPost] âš ï¸ SCROLL FAILED after 3 attempts for ${entryPoint}!`,
                `\n  Container: ${containerName}`,
        ...
        console.log(
          ...
          `\n  Early scroll (before DOM ops): container=${earlyScrollTop}px, scrollEl=${earlyActualScrollTop}px${earlyActualScrollTop > 0 ? ' âš ï¸ WAS SCROLLED' : ''}`,
          ...
          requestAnimationFrame(() => {
            ...
            setTimeout(() => {
              requestAnimationFrame(() => {
                ...
                console.log(
                  `[openPost] ${success ? 'âœ… SUCCESS' : 'âŒ FAILED'}: Scroll sequence complete for ${entryPoint}`,
                  `\n  Container: ${containerName}`,
    ...
    async function initMap(){
      // Call the map.js initMap function
    ...
    // Legacy initMap code removed - now handled by map.js
          container:'map',
...
// === Pill hooks (safe) ===
        ...
        const applyStyleAdjustments = () => {
          const ensurePlaceholderSprites = window.ensurePlaceholderSprites;
        const applyNightSky = window.applyNightSky;
        const patchMapboxStyleArtifacts = window.patchMapboxStyleArtifacts;
        const whenStyleReady = window.whenStyleReady;
        const map = getMap();
        if(map){
          try{ if(ensurePlaceholderSprites) ensurePlaceholderSprites(map); }catch(err){}
          if(applyNightSky) applyNightSky(map);
          if(patchMapboxStyleArtifacts) patchMapboxStyleArtifacts(map);
        }
        };
        if(map && whenStyleReady) whenStyleReady(map, applyStyleAdjustments);
