...
// === Added Confirm Password & Database Registration ===
(function integrateDatabaseRegistration(){
  const registerPanel = document.getElementById('memberRegisterPanel');
  if(!registerPanel) return;
  ...
  if(form){
    form.addEventListener('submit', async e => {
      ...
      if(action === 'register'){
        ...
        const displayName = registerPanel.querySelector('input[name="display_name"]').value;
        const email = registerPanel.querySelector('input[name="email"]').value;
        const avatarUrl = registerPanel.querySelector('input[name="avatar_url"]')?.value || '';
        try {
          const res = await fetch('add-member.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ display_name: displayName, email: email, password: pwd1.value, avatar_url: avatarUrl })
          });
...
// === End Added Confirm Password & Database Registration ===
