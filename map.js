...
const MapModule = (function() {
  ...
  // High-Density Layers (Mapbox native — commented out, replaced by HTML markers)
  ...
  // Mapbox native layer config (commented out, replaced by HTML markers)
  // function getDotSize() { return (window.App && typeof App.getConfig === 'function') ? App.getConfig('markerDotSize') : 8; }
  // function getDotStrokeWidth() { return (window.App && typeof App.getConfig === 'function') ? App.getConfig('markerDotStroke') : 2; }
  // function getIconDotSize() { return (window.App && typeof App.getConfig === 'function') ? App.getConfig('markerIconSize') : 30; }
  ...
  // let hoveredPostId = null; // Mapbox native layers (commented out)
  ...
  function injectMapCardStyles() {
    ...
    const css = `
      ...
      /* ============================================================
         MAP DOTS (colored circle for standard overflow markers)
         ============================================================ */
      .map-card-appearance-dot .map-card-pill { display: none; }
      .map-card-appearance-dot .map-dot-ring {
        position: absolute;
        left: -10px;
        top: -10px;
        width: 20px;
      ...
      .map-card-appearance-dot .map-dot-fill {
        position: absolute;
        left: -7.5px;
        top: -7.5px;
        width: 15px;
      ...
      .map-card-appearance-dot.is-hovered .map-dot-ring {
        background: #172554;
        outline: 1px solid var(--blue-500);
        outline-offset: -1px;
      }
      @media (hover: hover) and (pointer: fine) {
        .map-card-appearance-dot:hover .map-dot-ring {
          background: #172554;
          outline: 1px solid var(--blue-500);
          outline-offset: -1px;
        }
      ...
      .map-card-appearance-dot.is-active .map-dot-ring,
      .map-card-appearance-dot.is-active .map-dot-fill { display: none; }

      /* ============================================================
         MAP ICONS (subcategory icon for premium/featured overflow)
         ============================================================ */
      .map-card-appearance-icon .map-card-pill { display: none; }
      .map-card-appearance-icon .map-icon-ring {
        position: absolute;
        left: -20px;
        top: -20px;
        width: 40px;
      ...
      .map-card-appearance-icon .map-icon-image {
        position: absolute;
        left: -15px;
        top: -15px;
        width: 30px;
      ...
      .map-card-appearance-icon.is-hovered .map-icon-ring {
        background: #172554;
        outline: 2px solid var(--blue-500);
        outline-offset: -2px;
      }
      @media (hover: hover) and (pointer: fine) {
        .map-card-appearance-icon:hover .map-icon-ring {
          background: #172554;
          outline: 2px solid var(--blue-500);
          outline-offset: -2px;
        }
      ...
      .map-card-appearance-icon.is-active .map-icon-ring,
      .map-card-appearance-icon.is-active .map-icon-image { display: none; }
  ...
  function onMapLoad() {
    ...
    requestAnimationFrame(function() {
      ...
      App.whenStartupSettingsReady().then(function() {
        ...
        // Mapbox native high-density layers (commented out, replaced by HTML markers)
  ...
  /* ==================================================================
     MAPBOX NATIVE LAYERS (commented out — replaced by HTML markers)
     Kept for reference in case Mapbox native is needed for scale.
     ================================================================== */
  // function initHighDensityLayers() { ... }
  // function updateHighDensityData(geojson) { ... }
  // function ensureIconLoaded(iconId, iconUrl) { ... }
  // See backup for full implementation.
  ...
  /**
   * Create a dot marker (piggybacks off createMapCardMarker)
   */
  function createMapDotMarker(post, lng, lat, color) {
    if (!color) throw new Error('[Map] createMapDotMarker: color missing for post ID ' + post.id);
    var entry = createMapCardMarker(post, lng, lat);
    if (!entry) return null;
    var ring = document.createElement('div');
    ring.className = 'map-dot-ring';
    var fill = document.createElement('div');
    fill.className = 'map-dot-fill';
    fill.style.backgroundColor = color;
    entry.element.appendChild(ring);
    entry.element.appendChild(fill);
    entry.element.classList.add('map-card-appearance-dot');
    var hiddenIcon = entry.element.querySelector('.map-card-icon');
    if (hiddenIcon) hiddenIcon.removeAttribute('src');
    return entry;
  }

  /**
   * Create an icon marker (piggybacks off createMapCardMarker)
   */
  function createMapIconMarker(post, lng, lat) {
    if (!post.iconUrl) throw new Error('[Map] createMapIconMarker: iconUrl missing for post ID ' + post.id);
    var entry = createMapCardMarker(post, lng, lat);
    if (!entry) return null;
    var ring = document.createElement('div');
    ring.className = 'map-icon-ring';
    var img = document.createElement('img');
    img.className = 'map-icon-image';
    img.src = post.iconUrl;
    img.alt = '';
    entry.element.appendChild(ring);
    entry.element.appendChild(img);
    entry.element.classList.add('map-card-appearance-icon');
    var hiddenIcon = entry.element.querySelector('.map-card-icon');
    if (hiddenIcon) hiddenIcon.removeAttribute('src');
    return entry;
  }
  ...
  function onMapCardHoverByPostId(postId, isHovering) {
    ...
    // Mapbox native glow (commented out, replaced by HTML markers)
  ...
  return {
    ...
    // updateHighDensityData, // Mapbox native (commented out)
    createMapDotMarker,
    createMapIconMarker,
