...
/* =====================================================================================
 * FunMap Registration Patch — minimal, non-destructive
 * - Validates display_name, email, password+confirm before enabling button
 * - Sends POST JSON to /gateway.php?action=add-member
 * - Auto-logs in via /gateway.php?action=verify-login on success
 * - Resets state on logout
 * This block is self-contained and won't modify existing behavior unless
 * the relevant DOM elements are present.
 * ===================================================================================== */
(function(){
  if (window.__funmapRegistrationPatched) return;
  window.__funmapRegistrationPatched = true;

  function $(sel, root){ return (root||document).querySelector(sel); }
  function emailValid(e){ return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(e||'').trim()); }

  function findRegisterElements(){
    const displayName = $('#memberRegisterDisplayName') || $('#registerDisplayName') || $('[name="display_name"]');
    const email = $('#memberRegisterEmail') || $('#registerEmail') || $('[name="email"]');
    const pass = $('#memberRegisterPassword') || $('#registerPassword') || $('[name="password"]');
    const confirm = $('#memberRegisterConfirmPassword') || $('#registerConfirmPassword') || $('[name="confirm_password"]');
    let createBtn = document.querySelector('.member-auth [data-action="register"] .primary-action') 
                 || document.querySelector('.member-auth .primary-action.member-auth-submit')
                 || document.querySelector('[data-create-account]');
    const msg = $('.member-auth-message') || $('#memberAuthMessage') || $('#registerMessage');
    const logoutBtn = document.querySelector('.member-auth [data-action="logout"] .primary-action')
                   || document.querySelector('[data-action="logout"]');
    return { displayName, email, pass, confirm, createBtn, msg, logoutBtn };
  }
  ...
  function validateAll(els){
    const nameOK = !!(els.displayName && els.displayName.value.trim());
    const emailOK = !!(els.email && emailValid(els.email.value));
    const passOK = !!(els.pass && String(els.pass.value||'').length > 0);
    const confirmOK = !!(els.confirm && (els.pass.value === els.confirm.value) && String(els.confirm.value||'').length > 0);
  ...
  async function postJson(url, data){
    ...
    const text = await res.text();
    let json;
    try { json = JSON.parse(text); } catch(e){ json = { success:false, error:'Invalid server response', _raw:text }; }
    return { ok: res.ok, status: res.status, json };
  }

  async function handleRegister(els, e){
    if (e && typeof e.preventDefault === 'function') e.preventDefault();
    if (!validateAll(els)){
      setMessage(els.msg, 'Please complete all fields correctly.');
      return;
    }
    const payload = {
      ...
      confirm_password: els.confirm.value
    };
    if (els.createBtn) els.createBtn.disabled = true;
    try {
      const { json } = await postJson('/gateway.php?action=add-member', payload);
      if (json && json.success){
        setMessage(els.msg, (json.message || 'Account created.') + ' Logging you in…');
        const login = await postJson('/gateway.php?action=verify-login', { email: payload.email, password: payload.password });
        if (login.json && login.json.success){
          setMessage(els.msg, 'Welcome! You are now logged in.');
          try {
            if (typeof window.onLoginSuccess === 'function'){ window.onLoginSuccess(login.json); }
          } catch(_){}
        } else {
          setMessage(els.msg, (login.json && (login.json.error||login.json.message)) || 'Login failed after registration.');
        }
      } else {
        const err = (json && (json.error||json.message)) || 'Registration failed.';
        setMessage(els.msg, err);
      }
    } catch(err){
      setMessage(els.msg, 'Server error. Please try again.');
    } finally {
      if (els.createBtn) els.createBtn.disabled = !validateAll(els);
    }
  }

  function arm(){
    const els = findRegisterElements();
    if (!els.createBtn || !els.displayName || !els.email || !els.pass || !els.confirm){
      return; // DOM not present; do nothing
    }
    els.createBtn.disabled = !validateAll(els);

    const onInput = () => { els.createBtn.disabled = !validateAll(els); };
    [els.displayName, els.email, els.pass, els.confirm].forEach(inp => {
      if (inp) inp.addEventListener('input', onInput, { passive: true });
      if (inp && inp.name === 'email'){ inp.addEventListener('blur', onInput, { passive: true }); }
    });

    els.createBtn.addEventListener('click', handleRegister.bind(null, els));

    if (els.logoutBtn){
      els.logoutBtn.addEventListener('click', () => {
        try {
          if (els.displayName) els.displayName.value = '';
