...
### Post Deletion
- Soft delete: set `visibility = 'deleted'`, `deleted_at = NOW()`. Post is immediately invisible everywhere on the site (same as `hidden`).
- Hard delete at day 30 (cron): collect all image URLs from `post_media` → delete from Bunny CDN, then hard DELETE the `posts` row. All child tables cascade automatically: `post_map_cards`, `post_amenities`, `post_item_pricing`, `post_sessions`, `post_ticket_pricing`, `post_links`, `post_media`, `post_revisions`, `commissions`, `moderation_log`.
- `transactions`: leave untouched.
- Warning when paid credit time remains: wording to be decided — not "you're throwing your money away" but something considered. Decision deferred.

### Account Deletion
- Member must delete all active posts before deleting their account. This gate already exists (`msg_delete_blocked_active_posts`).
- Requires email verification code before proceeding (same flow as registration verification).
- Soft delete: set `members.deleted_at = NOW()`, `members.hidden = 1`. Account invisible immediately.
- Hard delete at day 30 (cron): send email 711 first, then collect all post images + avatar from Bunny CDN → delete. Hard DELETE all `posts` (cascades handle children). Hard DELETE `member_tokens`, `member_settings`, `members` row. Update `emails_sent.username` to "Deleted Member". Leave `transactions` untouched.
- Works identically for admins (same table structure).

### Confirmation Dialog Wording
- `msg_confirm_delete_account` (ID 253) currently says "This action cannot be undone." — this is wrong, it can be undone within 30 days. Needs rewording.

### Grace Period UX (login during 30-day window)
- Logging in must NOT cancel the deletion — that would be accidental.
- When a member with `deleted_at` set logs in, show a modal: displays the countdown timer, offers an explicit "Restore Account" option, and an "Acknowledge" option to continue without restoring.
- If they acknowledge, they can browse their account as normal during the grace period, but certain actions are blocked (payments, new posts, etc. — exact restrictions to be decided).
- Explicit "Cancel Deletion" / "Restore Account" action is the only way to stop the deletion.
