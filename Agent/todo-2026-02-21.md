...
i. **Rate limiting** — before sending, check `emails_sent` for emails sent to the same address in the last 24 hours; abort if over threshold
j. **Unsubscribe link** — legally required in many countries; add one-click unsubscribe to all emails
k. **Bounce handling** — add a `bounced` flag to `members`; stop sending to bounced addresses
l. **Cron job throttling** — expiry/expired cron jobs must throttle to max 1 email/second and batch per member
m. **Sender reputation monitoring** — register `funmap.com` at Google Postmaster Tools and Microsoft SNDS (both free)
n. **Inactivity closure** — if a member has not logged in for 365 days, close their account automatically. Two emails required: template 710 (warning, sent at 335 days — "your account will be closed in 30 days unless you log in") and template 711 (closure confirmation, sent at 365 days). Security rationale: email providers recycle dormant addresses; a live FunMap account tied to a recycled email is a takeover risk via password reset. Requires `last_login_at` column on `members` and `admins` tables, a daily cron job, and the same soft-delete/hard-delete logic as account deletion (708/709). Logging in at any point resets the 365-day clock.
...
The settings tab always opens scrolled down slightly (to the 3rd field, currently website_url). Root cause not found after exhaustive search: no `autofocus`, no `.focus()`, no `scrollIntoView`, no `scrollTop` assignment fires during init. `document.activeElement` on open returns the admin header button, not any input — ruling out focus as the cause. Suspected: Chrome's native scrollTop restoration for inner scrollable divs within a session. Fix requires identifying the exact trigger; do not smother with a `scrollTop = 0` override until the real cause is known.
