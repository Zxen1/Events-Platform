...
l. [TEMPLATE DONE] **711** `msg_email_account_deleted` — Account permanently deleted — fires when cron hard-deletes the account at day 30. Cron trigger needed.
...
p. [TEMPLATE DONE] **715** `msg_email_inactivity_closed` — Inactivity closure — sent at 365 days; account closed. `last_login_at` column confirmed on both tables. Cron trigger needed. **Inactivity definition:** the 365-day clock is based on the most recent of: last login, last post created, last payment, or last post expiry date. If any post is still within its paid period, the account is not inactive. The clock only starts after the last paid post has also expired.
