...
The settings tab always opens scrolled to the 3rd field. No `autofocus`, `.focus()`, `scrollIntoView`, or `scrollTop` assignment found during init. `document.activeElement` on open is the admin button — not an input — ruling out focus.
Suspected: Chrome's native scrollTop restoration for inner scrollable divs. Do not fix with a `scrollTop = 0` override — find the real cause first.
