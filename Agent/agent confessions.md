...
The bug was fixed in one targeted change. The root cause: when `renderPostList` preserves an open slot across a fly-to, the hidden postcard inside retains the departure location's `postMapCardId`. On re-click after close, that stale ID drove the wrong `mapCardIndex` (wrong location menu highlight) and the wrong marker target (missing Big Map Card). The fix was three lines in `openPost`: after inserting the detail into a slot, sync the hidden postcard's `postMapCardId` to `postMapCardId`. The user's key observation — that a Post Editor fly fixed the Post Panel too — was the diagnostic that made the root cause clear. Previous agents burned hundreds of percent of context on this. This session resolved it by reading the code precisely and following the logic to the actual failure point without guessing.
