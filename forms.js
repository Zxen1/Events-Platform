(function(){
  ...
  function init(){
    ...
    let renderConfiguredFieldsPending = false;
      ...
      if(fields.length === 0){
        ...
        if (typeof window.renderForm === 'function') {
          ...
          // Render checkout options after form is rendered
          (async function(){
            try {
              // Get checkout options and site currency from admin settings
              let checkoutOptions = [];
              let siteCurrency = 'USD';
              
              if(window.adminSettings && window.adminSettings.checkout_options){
                checkoutOptions = window.adminSettings.checkout_options;
              } else if(window.formbuilderStateManager && window.formbuilderStateManager._snapshot && window.formbuilderStateManager._snapshot.checkoutOptions){
                checkoutOptions = window.formbuilderStateManager._snapshot.checkoutOptions;
              } else {
                  ...
                  if(response.ok){
                    ...
                    if(data.success && data.settings){
                      ...
                      if(data.settings.website_currency){
                        siteCurrency = data.settings.website_currency;
                      }
                    }
                  }
                } catch(err){
                  console.warn('Failed to load checkout options:', err);
                }
              }
              
              if(window.adminSettings && window.adminSettings.website_currency){
                siteCurrency = window.adminSettings.website_currency;
              }
              
              // Render checkout options before submit button
              if(postActions && checkoutOptions.length > 0){
                // Insert checkout options before the submit button
                const submitBtn = postActions.querySelector('#memberCreatePostBtn');
                if(submitBtn){
                  renderFormCheckoutOptions(postActions, checkoutOptions, siteCurrency);
                } else {
                  // If button doesn't exist yet, insert at end
                  renderFormCheckoutOptions(postActions, checkoutOptions, siteCurrency);
                }
              }
            } catch(err){
              console.warn('Failed to render checkout options:', err);
            }
          })();
        } else {
    ...
    // === Checkout Options Rendering for Forms ===
    function calculateDaysFromSubmissionToFinalSession(){
      // Calculate days from post submission date to final session date
      // This will be calculated from form data when available
      // For now, return null to indicate calculation needed
      // The actual calculation will happen when form is submitted or dates are entered
      return null;
    }
    
    function renderFormCheckoutOptions(container, checkoutOptions, siteCurrency, selectedDays = null){
      if(!container || !checkoutOptions || !Array.isArray(checkoutOptions) || checkoutOptions.length === 0){
        return;
      }
      
      // Clear existing checkout options
      const existing = container.querySelector('.form-checkout-options');
      if(existing) existing.remove();
      
      const checkoutWrapper = document.createElement('div');
      checkoutWrapper.className = 'form-checkout-options';
      
      const checkoutTitle = document.createElement('h3');
      checkoutTitle.className = 'form-checkout-title';
      checkoutTitle.textContent = 'Select Listing Duration';
      checkoutWrapper.appendChild(checkoutTitle);
      
      const checkoutGrid = document.createElement('div');
      checkoutGrid.className = 'form-checkout-grid';
      
      checkoutOptions.forEach((option, index) => {
        if(!option.is_active) return;
        
        const optionCard = document.createElement('div');
        optionCard.className = 'form-checkout-option-card';
        optionCard.dataset.optionId = option.id;
        
        const flagfallPrice = option.checkout_flagfall_price !== undefined ? parseFloat(option.checkout_flagfall_price) : 0;
        ...
        const currency = option.checkout_currency || siteCurrency || 'USD';
        
        // Calculate prices for 30 and 365 day options
        ...
        optionTitle.className = 'form-checkout-option-title';
        optionTitle.textContent = option.checkout_title || 'Listing Option';
        optionCard.appendChild(optionTitle);
        
        if(option.checkout_description){
          const optionDesc = document.createElement('div');
          optionDesc.className = 'form-checkout-option-description';
          optionDesc.textContent = option.checkout_description;
          optionCard.appendChild(optionDesc);
        }
        
        const durationOptions = document.createElement('div');
        durationOptions.className = 'form-checkout-duration-options';
        
        // 30 day option
        const option30 = document.createElement('button');
        option30.type = 'button';
        option30.className = 'form-checkout-duration-btn';
        option30.dataset.duration = '30';
        option30.dataset.optionId = option.id;
        option30.innerHTML = `
          <span class="duration-label">30 Days</span>
          <span class="duration-price">${currency} ${price30Days.toFixed(2)}</span>
        `;
        option30.addEventListener('click', function(){
          // Remove selected state from all buttons in this card
          optionCard.querySelectorAll('.form-checkout-duration-btn').forEach(btn => {
            btn.classList.remove('selected');
            btn.setAttribute('aria-pressed', 'false');
          });
          // Add selected state to clicked button
          option30.classList.add('selected');
          option30.setAttribute('aria-pressed', 'true');
          
          // Store selection
          if(!window.formCheckoutSelection) window.formCheckoutSelection = {};
          window.formCheckoutSelection.optionId = option.id;
          window.formCheckoutSelection.duration = 30;
          window.formCheckoutSelection.price = price30Days;
        });
        durationOptions.appendChild(option30);
        
        // 365 day option
        const option365 = document.createElement('button');
        option365.type = 'button';
        option365.className = 'form-checkout-duration-btn';
        option365.dataset.duration = '365';
        option365.dataset.optionId = option.id;
        option365.innerHTML = `
          <span class="duration-label">365 Days</span>
          <span class="duration-price">${currency} ${price365Days.toFixed(2)}</span>
        `;
        option365.addEventListener('click', function(){
          // Remove selected state from all buttons in this card
          optionCard.querySelectorAll('.form-checkout-duration-btn').forEach(btn => {
            btn.classList.remove('selected');
            btn.setAttribute('aria-pressed', 'false');
          ...
          // Add selected state to clicked button
          option365.classList.add('selected');
          option365.setAttribute('aria-pressed', 'true');
          
          // Store selection
          if(!window.formCheckoutSelection) window.formCheckoutSelection = {};
          window.formCheckoutSelection.optionId = option.id;
          window.formCheckoutSelection.duration = 365;
          window.formCheckoutSelection.price = price365Days;
        });
        durationOptions.appendChild(option365);
        
        optionCard.appendChild(durationOptions);
        checkoutGrid.appendChild(optionCard);
      });
      
      checkoutWrapper.appendChild(checkoutGrid);
      // Insert before submit button if it exists, otherwise append
      const submitBtn = container.querySelector('#memberCreatePostBtn');
      if(submitBtn){
        container.insertBefore(checkoutWrapper, submitBtn);
      } else {
        container.appendChild(checkoutWrapper);
      }
    }
    
    // Make function globally accessible for form preview
    window.renderFormCheckoutOptions = renderFormCheckoutOptions;
