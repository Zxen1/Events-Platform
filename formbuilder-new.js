...
(function() {
    ...
    /* --------------------------------------------------------------------------
       ACCORDION POSITION TRACKING SYSTEM
       Keeps buttons stationary when accordions above close downward
       -------------------------------------------------------------------------- */
    
    var accordionPositions = [];
    var isAnimating = false;
    var headerHeight = 36; // Height of accordion header
    
    function initializeAccordionPositions() {
        if (!container) return;
        accordionPositions = [];
        var accordions = container.querySelectorAll('.formbuilder-accordion');
        var currentTop = 0;
        
        accordions.forEach(function(accordion, index) {
            // Set initial top position
            accordion.style.top = currentTop + 'px';
            
            var body = accordion.querySelector('.formbuilder-accordion-body');
            var bodyHeight = 0;
            if (accordion.classList.contains('formbuilder-accordion--open') && body) {
                bodyHeight = body.offsetHeight;
            }
            
            accordionPositions[index] = {
                element: accordion,
                top: currentTop,
                isOpen: accordion.classList.contains('formbuilder-accordion--open'),
                bodyHeight: bodyHeight
            };
            
            // Calculate next position
            currentTop += headerHeight + bodyHeight + 3; // 3px margin-bottom
        });
        
        // Update container height
        var addCatBtn = container.querySelector('.formbuilder-add-category');
        if (addCatBtn) {
            addCatBtn.style.position = 'absolute';
            addCatBtn.style.top = currentTop + 'px';
            currentTop += 36 + 3; // button height + margin
        }
        
        container.style.minHeight = currentTop + 'px';
    }
    ...
    function updateAccordionPositions() {
        ...
        accordions.forEach(function(accordion, index) {
            if (accordionPositions[index]) {
                accordionPositions[index].top = currentTop;
                accordionPositions[index].isOpen = accordion.classList.contains('formbuilder-accordion--open');
                
                var body = accordion.querySelector('.formbuilder-accordion-body');
                var bodyHeight = 0;
                if (accordionPositions[index].isOpen && body) {
                    bodyHeight = body.offsetHeight;
                }
                accordionPositions[index].bodyHeight = bodyHeight;
    ...
    function isTouching(aboveIndex, belowIndex) {
        if (aboveIndex < 0 || belowIndex >= accordionPositions.length) return false;
        var aboveBottom = accordionPositions[aboveIndex].top + headerHeight;
        if (accordionPositions[aboveIndex].isOpen) {
            aboveBottom += accordionPositions[aboveIndex].bodyHeight;
        }
        return Math.abs(aboveBottom - accordionPositions[belowIndex].top) < 3;
    }
    ...
    function handleAccordionClick(clickedAccordion) {
        ...
        var clickedIndex = -1;
        for (var i = 0; i < accordionPositions.length; i++) {
            if (accordionPositions[i].element === clickedAccordion) {
                clickedIndex = i;
                break;
            }
        }
        ...
        var clickedTop = accordionPositions[clickedIndex].top;
        var wasOpen = accordionPositions[clickedIndex].isOpen;
        var willBeOpen = !wasOpen;
        
        // Calculate what will happen
        var hasAboveAction = false;
        var hasBelowDropAction = false;
        var hasBelowMoveUpAction = false;
        
        // Check if rectangle below is touching (drop down)
        if (clickedIndex < accordionPositions.length - 1) {
            var belowIndex = clickedIndex + 1;
            if (isTouching(clickedIndex, belowIndex)) {
                if (wasOpen && !willBeOpen) {
                    hasBelowDropAction = true; // Will create gap below
                }
            } else {
                hasBelowMoveUpAction = true; // Need to move up to touch
            }
        }
        
        // Close all other open accordions (accordion behavior - only one open at a time)
        // Check if any other accordions are open (above or below)
        var closingAccordionsAbove = [];
        for (var i = 0; i < accordionPositions.length; i++) {
            if (i !== clickedIndex && accordionPositions[i].isOpen) {
                // If it's above the clicked one, we need to animate it down
                if (i < clickedIndex) {
                    closingAccordionsAbove.push(i);
                    hasAboveAction = true;
                }
                // Close this accordion (but don't update positions yet - animation will handle it)
                accordionPositions[i].element.classList.remove('formbuilder-accordion--open');
            }
        }
        
        if (!hasAboveAction && !hasBelowDropAction && !hasBelowMoveUpAction) {
            // Just toggle, no animation needed
            return;
        }
        
        isAnimating = true;
        var aboveComplete = !hasAboveAction;
        var belowComplete = !hasBelowDropAction && !hasBelowMoveUpAction;
        ...
        function checkComplete() {
            if (aboveComplete && belowComplete) {
                ...
                // Scroll compensation - handle gaps
                handleScrollCompensation(clickedIndex);
            }
        }
    
    function handleScrollCompensation(clickedIndex) {
        if (!container) return;
        
        var clickedAccordion = accordionPositions[clickedIndex].element;
        var headerRect = clickedAccordion.querySelector('.formbuilder-accordion-header').getBoundingClientRect();
        var containerRect = container.getBoundingClientRect();
        
        // Check if header is visible in viewport
        var headerTop = headerRect.top - containerRect.top;
        var headerBottom = headerRect.bottom - containerRect.top;
        var viewportTop = container.scrollTop;
        var viewportBottom = container.scrollTop + container.clientHeight;
        
        // If header is above viewport, scroll to show it
        if (headerTop < viewportTop) {
            container.scrollTop = accordionPositions[clickedIndex].top;
        }
        // If there's a gap and header is visible, check if we need to adjust
        else if (headerTop >= viewportTop && headerTop < viewportBottom) {
            // Header is visible - check for gap below
            var body = clickedAccordion.querySelector('.formbuilder-accordion-body');
            if (body && clickedAccordion.classList.contains('formbuilder-accordion--open')) {
                var bodyRect = body.getBoundingClientRect();
                var gap = bodyRect.top - headerRect.bottom;
                
                // If gap is small and off-screen (above), adjust scroll to make content appear to touch
                if (gap > 0 && bodyRect.top < containerRect.top) {
                    // Content is above viewport - scroll up to make it appear to touch header
                    container.scrollTop = accordionPositions[clickedIndex].top;
                }
            }
        }
        ...
        // Handle moving accordions above down (when they close)
        if (hasAboveAction && closingAccordionsAbove.length > 0) {
            var totalHeightToAdd = 0;
            
            // Calculate total height of closing accordions above
            closingAccordionsAbove.forEach(function(i) {
                totalHeightToAdd += accordionPositions[i].bodyHeight;
            });
            
            if (totalHeightToAdd > 0) {
                var speed = 8;
                var currentOffset = 0;
                ...
                function animateAbove() {
                    ...
                    if (currentOffset >= totalHeightToAdd) {
                        currentOffset = totalHeightToAdd;
                        
                        // Move all accordions above down
                        for (var i = 0; i < clickedIndex; i++) {
                            var newTop = accordionPositions[i].top + currentOffset;
                            accordionPositions[i].element.style.top = newTop + 'px';
                            accordionPositions[i].top = newTop;
                        }
                    ...
                    // Move all accordions above down together
                    for (var i = 0; i < clickedIndex; i++) {
                        var newTop = accordionPositions[i].top + currentOffset;
                        accordionPositions[i].element.style.top = newTop + 'px';
                    }
        ...
        // Handle dropping accordions below (when clicked accordion closes)
        if (hasBelowDropAction) {
            var dropDistance = wasOpen ? accordionPositions[clickedIndex].bodyHeight : 0;
            var speed = 8;
            var currentOffset = 0;
            ...
            function animateBelowDrop() {
                ...
                if (currentOffset >= dropDistance) {
                    ...
                    // Move all accordions below down
                    for (var i = clickedIndex + 1; i < accordionPositions.length; i++) {
                        var newTop = accordionPositions[i].top + currentOffset;
                        accordionPositions[i].element.style.top = newTop + 'px';
                        accordionPositions[i].top = newTop;
                    }
                ...
                // Move all accordions below down together
                for (var i = clickedIndex + 1; i < accordionPositions.length; i++) {
                    var newTop = accordionPositions[i].top + currentOffset;
                    accordionPositions[i].element.style.top = newTop + 'px';
                }
        ...
        // Handle moving accordions below up (when there's a gap)
        if (hasBelowMoveUpAction) {
            var firstBelowIndex = clickedIndex + 1;
            var targetTop = clickedTop + headerHeight;
            if (willBeOpen) {
                // Need to measure body height after it opens
                var body = clickedAccordion.querySelector('.formbuilder-accordion-body');
                if (body) {
                    targetTop += body.offsetHeight;
                }
            }
            var distanceToMove = accordionPositions[firstBelowIndex].top - targetTop;
            ...
            if (distanceToMove > 0) {
                ...
                function animateBelowMoveUp() {
                    ...
                    if (currentOffset >= distanceToMove) {
                        ...
                        // Move all accordions below up
                        for (var i = firstBelowIndex; i < accordionPositions.length; i++) {
                            var newTop = accordionPositions[i].top - currentOffset;
                            accordionPositions[i].element.style.top = newTop + 'px';
                            accordionPositions[i].top = newTop;
                        }
                    ...
                    // Move all accordions below up together
                    for (var i = firstBelowIndex; i < accordionPositions.length; i++) {
                        var newTop = accordionPositions[i].top - currentOffset;
                        accordionPositions[i].element.style.top = newTop + 'px';
                    }
    ...
    function renderForm(formData) {
        ...
        // Initialize accordion positions after rendering
        setTimeout(function() {
            initializeAccordionPositions();
        }, 0);
    ...
    function buildCategoryAccordion(cat, categoryIconPaths, subcategoryIconPaths, fieldsets) {
        ...
        header.addEventListener('click', function(e) {
            ...
            // Update positions before toggle
            updateAccordionPositions();
            
            var wasOpen = accordion.classList.contains('formbuilder-accordion--open');
            
            // Handle accordion animation (closes others, animates positions)
            handleAccordionClick(accordion);
            
            // Toggle the open state
            if (wasOpen) {
                accordion.classList.remove('formbuilder-accordion--open');
            } else {
                accordion.classList.add('formbuilder-accordion--open');
            }
            
            // Update positions after toggle to account for body height change
            setTimeout(function() {
                updateAccordionPositions();
