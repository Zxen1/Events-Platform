...
/* ============================================================================
   ...
   Calendar + session selection for open posts.
   Source: Migrated from index.js buildDetail() session menu functionality.
   ============================================================================ */
...
const SessionMenuComponent = (function(){
    ...
    function formatDateDisplay(dateEntry, includeYear) {
        if (!dateEntry || !dateEntry.date) return '';
        if (includeYear) {
            var parsed = parseSessionDate(dateEntry.full);
            return dateEntry.date + ', ' + parsed.getFullYear();
        }
        return dateEntry.date;
    }
    
    /**
     * Create a session menu component
     * @param {HTMLElement} containerEl - Container to render into
     * @param {Object} options - Configuration options
     * @param {HTMLElement} options.buttonEl - The session button element
     * @param {HTMLElement} options.menuEl - The session menu dropdown element
     * @param {HTMLElement} options.infoEl - The session info display element
     * @param {HTMLElement} options.calendarContainerEl - The calendar container element
     * @param {HTMLElement} options.sessionOptsEl - The session options container element
     * @param {Array} options.sessions - Array of session objects {date, full, time}
     * @param {string} options.priceText - Default price text to display
     * @param {function} options.onSelect - Callback when session selected
     * @param {function} options.shouldShowExpired - Function returning whether to show expired sessions
     */
    function create(containerEl, options) {
        options = options || {};
        var buttonEl = options.buttonEl || null;
        var menuEl = options.menuEl || null;
        var infoEl = options.infoEl || null;
        var calendarContainerEl = options.calendarContainerEl || null;
        var sessionOptsEl = options.sessionOptsEl || null;
        var sessions = options.sessions || [];
        var priceText = options.priceText || '';
        var onSelect = options.onSelect || null;
        var shouldShowExpired = options.shouldShowExpired || function() { return false; };
        
        var selectedIndex = null;
        var calendarEl = null;
        var calScrollEl = null;
        var lastClickedCell = null;
        var initialized = false;
        var visibleDateEntries = [];
        var dateStrings = [];
        var allowedSet = new Set();
        var months = [];
        var defaultInfoHTML = '';
        var sessionHasMultiple = false;
        ...
        function computeVisibleSessions() {
            var showExpired = shouldShowExpired();
            var threshold = sessionThresholdDate();
            return sessions
                .map(function(d, i) { return {d: d, i: i}; })
                .filter(function(entry) {
                    if (!entry.d || typeof entry.d.full !== 'string') return false;
                    if (showExpired) return true;
                    var parsed = parseSessionDate(entry.d.full);
                    return parsed instanceof Date && !isNaN(parsed.getTime()) && parsed >= threshold;
                });
        }
        
        function recomputeVisibleDateData() {
            visibleDateEntries = computeVisibleSessions();
            
            var seen = new Set();
            var uniqueEntries = [];
            visibleDateEntries.forEach(function(entry) {
                if (!entry.d || typeof entry.d.full !== 'string') return;
                if (seen.has(entry.d.full)) return;
                var parsed = parseSessionDate(entry.d.full);
                if (!(parsed instanceof Date) || isNaN(parsed.getTime())) return;
                seen.add(entry.d.full);
                uniqueEntries.push({ iso: entry.d.full, date: parsed });
            });
            
            dateStrings = uniqueEntries.map(function(e) { return e.iso; });
            allowedSet = new Set(dateStrings);
            
            if (uniqueEntries.length) {
                var minDate = new Date(uniqueEntries[0].date.getTime());
                var maxDate = new Date(uniqueEntries[uniqueEntries.length - 1].date.getTime());
        ...
        function refreshDefaultSessionInfo() {
            var visible = visibleDateEntries;
            sessionHasMultiple = visible.length > 1;
            if (!infoEl) {
                defaultInfoHTML = '';
                return;
            }
            var suffix = '<span style="display:inline-block;margin-left:10px;">(Select Session)</span>';
            if (visible.length) {
                var firstDate = visible[0].d;
                var lastDate = visible[visible.length - 1].d;
                var firstIncludeYear = parseSessionDate(firstDate.full).getFullYear() !== currentYear;
                var lastIncludeYear = parseSessionDate(lastDate.full).getFullYear() !== currentYear;
                var rangeText = formatDateDisplay(firstDate, firstIncludeYear) + ' - ' + formatDateDisplay(lastDate, lastIncludeYear);
                defaultInfoHTML = '<div>' + (priceText ? priceText + ' | ' : '') + '📅 ' + rangeText + suffix + '</div>';
            } else if (sessions.length) {
                defaultInfoHTML = '<div>' + priceText + suffix + '</div>';
            } else {
                defaultInfoHTML = '';
            }
            infoEl.innerHTML = defaultInfoHTML;
        }
        
        function markSelected() {
            if (!calendarEl) return;
            var days = calendarEl.querySelectorAll('.session-calendar-day');
            days.forEach(function(d) { d.classList.remove('session-calendar-day--selected'); });
            if (selectedIndex !== null && sessions[selectedIndex]) {
                var dt = sessions[selectedIndex];
                var cell = calendarEl.querySelector('.session-calendar-day[data-iso="' + dt.full + '"]');
                if (cell) cell.classList.add('session-calendar-day--selected');
            }
        }
        
        function scrollCalendarToMonth(dt, smooth) {
            if (!dt || !calendarEl || !calScrollEl) return null;
            var cell = calendarEl.querySelector('.session-calendar-day[data-iso="' + dt.full + '"]');
            if (!cell) return null;
            var monthEl = cell.closest('.session-calendar-month');
            if (!monthEl) return null;
            var targetLeft = monthEl.offsetLeft;
            var maxLeft = Math.max(0, calScrollEl.scrollWidth - calScrollEl.clientWidth);
            targetLeft = Math.min(Math.max(targetLeft, 0), maxLeft);
            if (smooth) {
                calScrollEl.scrollTo({ left: targetLeft, behavior: 'smooth' });
            } else {
                calScrollEl.scrollLeft = targetLeft;
            }
            return { targetLeft: targetLeft };
        }
        
        function selectSession(i) {
            selectedIndex = (typeof i === 'number' && !isNaN(i)) ? i : null;
            
            // Update session options button selection
            if (sessionOptsEl) {
                var buttons = sessionOptsEl.querySelectorAll('button');
                buttons.forEach(function(b) { b.classList.remove('session-menu-option--selected'); });
                if (selectedIndex !== null) {
                    var btn = sessionOptsEl.querySelector('button[data-index="' + selectedIndex + '"]');
                    if (btn) btn.classList.add('session-menu-option--selected');
                }
            }
            
            var dt = selectedIndex !== null ? sessions[selectedIndex] : null;
            
            if (dt) {
                // Update info display
                if (infoEl) {
                    var includeYear = parseSessionDate(dt.full).getFullYear() !== currentYear;
                    infoEl.innerHTML = '<div><strong>' + formatDateDisplay(dt, includeYear) + ' ' + dt.time + '</strong></div>';
                }
                // Update button display
                if (buttonEl) {
                    var includeYear2 = parseSessionDate(dt.full).getFullYear() !== currentYear;
                    buttonEl.innerHTML = '<div class="open-post-image-navpreview open-post-image-navpreview--calendar"></div>' +
                        '<span class="session-menu-text-date">' + formatDateDisplay(dt, includeYear2) + '</span>' +
                        '<span class="session-menu-text-time">' + dt.time + '</span>' +
                        (sessionHasMultiple ? '<span class="open-post-icon-arrow" aria-hidden="true"></span>' : '');
                }
                markSelected();
                scrollCalendarToMonth(dt, true);
                
                // Call onSelect callback
                if (onSelect) onSelect(dt, selectedIndex);
            } else {
                // Reset to default
                if (infoEl) {
                    infoEl.innerHTML = defaultInfoHTML;
                ...
                if (buttonEl) {
                    buttonEl.innerHTML = '<div class="open-post-image-navpreview open-post-image-navpreview--calendar"></div>' +
                        (sessionHasMultiple ? 'Select Session<span class="open-post-icon-arrow" aria-hidden="true"></span>' : 'Select Session');
        ...
        function showTimePopup(matches) {
            ...
            var existing = calendarContainerEl.querySelector('.session-menu-timepopup');
            if (existing) existing.remove();
            
            var popup = document.createElement('div');
            popup.className = 'session-menu-timepopup';
            var list = document.createElement('div');
            list.className = 'session-menu-timepopup-list';
            matches.forEach(function(m) {
                var btn = document.createElement('button');
                btn.className = 'session-menu-timepopup-button';
                btn.dataset.index = m.i;
                btn.textContent = m.d.time;
                btn.addEventListener('click', function() {
                    selectSession(parseInt(btn.dataset.index, 10));
                    popup.remove();
                });
                list.appendChild(btn);
            });
            popup.appendChild(list);
            calendarContainerEl.appendChild(popup);
            
            // Position popup near last clicked cell
            if (lastClickedCell) {
                var rect = lastClickedCell.getBoundingClientRect();
                var containerRect = calendarContainerEl.getBoundingClientRect();
                var dateCenterX = rect.left + rect.width / 2 - containerRect.left;
                var containerCenterX = calendarContainerEl.clientWidth / 2;
                ...
                if (dateCenterX < containerCenterX) {
                    ...
                    popup.style.left = (rect.left - containerRect.left - popup.offsetWidth - 4) + 'px';
                }
            }
        ...
        function renderMonth(monthDate, cal) {
            var monthEl = document.createElement('div');
            monthEl.className = 'session-calendar-month';
            
            var header = document.createElement('div');
            header.className = 'session-calendar-header';
            header.textContent = monthDate.toLocaleDateString('en-GB', { month: 'long', year: 'numeric' });
            monthEl.appendChild(header);
            
            var grid = document.createElement('div');
            grid.className = 'session-calendar-grid';
            
            var weekdays = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
            weekdays.forEach(function(wd) {
                var w = document.createElement('div');
                w.className = 'session-calendar-weekday';
                w.textContent = wd;
                grid.appendChild(w);
            });
            ...
            for (var i = 0; i < totalCells; i++) {
                var cell = document.createElement('div');
                cell.className = 'session-calendar-day';
                var dayNum = i - startDow + 1;
                
                if (i < startDow || dayNum > daysInMonth) {
                    cell.classList.add('session-calendar-day--empty');
                } else {
                    cell.textContent = dayNum;
                    ...
                    if (allowedSet.has(iso)) {
                        cell.classList.add('session-calendar-day--available');
                        cell.addEventListener('mousedown', function() { lastClickedCell = this; }.bind(cell));
                        cell.addEventListener('click', function() {
                            var clickedIso = this.dataset.iso;
                            var matches = visibleDateEntries.filter(function(entry) {
                                return entry.d.full === clickedIso;
                            });
                            if (matches.length === 1) {
                    ...
                    if (isToday(dateObj)) {
                        cell.classList.add('session-calendar-day--today');
                    }
                }
                grid.appendChild(cell);
            }
        ...
        function buildCalendar() {
            ...
            calScrollEl = document.createElement('div');
            calScrollEl.className = 'session-calendar-scroll';
            
            calendarEl = document.createElement('div');
            calendarEl.className = 'session-calendar';
            
            months.forEach(function(monthDate) {
                renderMonth(monthDate, calendarEl);
            });
            
            calScrollEl.appendChild(calendarEl);
            calendarContainerEl.appendChild(calScrollEl);
            
            // Horizontal scroll with mouse wheel
            calScrollEl.addEventListener('wheel', function(e) {
                var delta = Math.abs(e.deltaX) > Math.abs(e.deltaY) ? e.deltaX : e.deltaY;
                if (delta !== 0) {
                    calScrollEl.scrollLeft += delta;
                    e.preventDefault();
                }
            }, { passive: false });
        ...
        function buildSessionOptions() {
            if (!sessionOptsEl) return;
            sessionOptsEl.innerHTML = '';
            
            visibleDateEntries.forEach(function(entry) {
                var btn = document.createElement('button');
                btn.className = 'session-menu-option';
                btn.dataset.index = entry.i;
                var includeYear = parseSessionDate(entry.d.full).getFullYear() !== currentYear;
                btn.innerHTML = '<span class="session-menu-option-date">' + formatDateDisplay(entry.d, includeYear) + '</span>' +
                    '<span class="session-menu-option-time">' + entry.d.time + '</span>';
                btn.addEventListener('click', function() {
                    selectSession(parseInt(btn.dataset.index, 10));
                });
                sessionOptsEl.appendChild(btn);
            });
        }
        ...
        function openMenu() {
            ...
            if (selectedIndex !== null && sessions[selectedIndex]) {
                requestAnimationFrame(function() {
                    scrollCalendarToMonth(sessions[selectedIndex], false);
                });
            }
        }
        ...
        function init() {
            ...
            sessions.sort(function(a, b) {
                var fullA = (a && a.full) || '';
                var fullB = (b && b.full) || '';
                var fullCompare = fullA.localeCompare(fullB);
            ...
            // Auto-select first session if only one visible
            if (visibleDateEntries.length === 1) {
                selectSession(visibleDateEntries[0].i);
            } else if (visibleDateEntries.length > 1) {
                // Multiple sessions - show select prompt
                if (buttonEl) {
                    buttonEl.innerHTML = '<div class="open-post-image-navpreview open-post-image-navpreview--calendar"></div>' +
        ...
        function refresh() {
            ...
            // Check if selected session is still visible
            var selectedIsVisible = visibleDateEntries.some(function(entry) {
                return entry.i === selectedIndex;
            });
            ...
            if (!sessionHasMultiple && visibleDateEntries.length === 1) {
                selectSession(visibleDateEntries[0].i);
            } else if (selectedIndex === null) {
                if (infoEl) infoEl.innerHTML = defaultInfoHTML;
                if (buttonEl) {
                    buttonEl.innerHTML = '<div class="open-post-image-navpreview open-post-image-navpreview--calendar"></div>' +
                        (sessionHasMultiple ? 'Select Session<span class="open-post-icon-arrow" aria-hidden="true"></span>' : 'Select Session');
                }
        ...
        function destroy() {
            if (calendarContainerEl) calendarContainerEl.innerHTML = '';
            if (sessionOptsEl) sessionOptsEl.innerHTML = '';
            initialized = false;
            sessions = [];
            visibleDateEntries = [];
        }
