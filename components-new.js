...
const LocationWallpaperComponent = (function() {
    ...
    function getWallpaperMode() {
        ...
        // Wallpaper is now controlled entirely by member's Animation setting in Profile tab.
        // TODO: Decide whether to restore admin control, keep member-only, or combine both.
        ...
        try {
            if (window.App && typeof App.getState === 'function') {
                ...
                if (v === 'orbit' || v === 'still' || v === 'off') adminMode = v;
            }
        } catch (e) {}
        if (adminMode === 'off') return 'off';
        */
        
        // Check member's animation preference
        var memberPref = 'full';
        try {
            if (window.MemberModule && typeof MemberModule.getAnimationPreference === 'function') {
                memberPref = MemberModule.getAnimationPreference() || 'full';
            } else {
                var stored = localStorage.getItem('animation_preference');
                if (stored) memberPref = stored;
            }
        } catch (e2) {}
