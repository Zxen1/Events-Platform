...
/* ============================================================================
   ...
   Reusable 3-button dialog for unsaved changes prompts.
   Used by both admin and member panels.
   
   Usage:
     const result = await UnsavedChangesDialogComponent.show({
       titleText: 'Unsaved Changes',
       messageText: 'You have unsaved changes. Save before closing?',
       cancelLabel: 'Cancel',
       saveLabel: 'Save',
       discardLabel: 'Discard Changes',
       onCancel: function() { /* close dialog */ },
       onSave: function() { /* save and close */ },
       onDiscard: function() { /* discard and close */ }
     });
   ============================================================================ */

const UnsavedChangesDialogComponent = (function() {
    var overlayEl = null;
    ...
    function ensureOverlay() {
        ...
        var overlay = document.createElement('div');
        overlay.className = 'component-unsaved-dialog-overlay';
        overlay.setAttribute('aria-hidden', 'true');
        overlay.setAttribute('tabindex', '-1');
        
        var dialog = document.createElement('div');
        dialog.className = 'component-unsaved-dialog';
        dialog.setAttribute('role', 'dialog');
        dialog.setAttribute('aria-modal', 'true');
        dialog.setAttribute('aria-labelledby', 'unsavedDialogTitle');
        dialog.setAttribute('aria-describedby', 'unsavedDialogMessage');
        
        var title = document.createElement('h3');
        title.id = 'unsavedDialogTitle';
        title.className = 'component-unsaved-dialog-title';
        
        var message = document.createElement('p');
        message.id = 'unsavedDialogMessage';
        message.className = 'component-unsaved-dialog-message';
        
        var actions = document.createElement('div');
        actions.className = 'component-unsaved-dialog-actions';
        
        var cancelBtn = document.createElement('button');
        cancelBtn.type = 'button';
        cancelBtn.className = 'component-unsaved-dialog-button component-unsaved-dialog-button--cancel component-unsaved-dialog-button--primary';
        cancelBtn.dataset.role = 'cancel';
        
        var saveBtn = document.createElement('button');
        saveBtn.type = 'button';
        saveBtn.className = 'component-unsaved-dialog-button component-unsaved-dialog-button--save';
        saveBtn.dataset.role = 'save';
        
        var discardBtn = document.createElement('button');
        discardBtn.type = 'button';
        discardBtn.className = 'component-unsaved-dialog-button component-unsaved-dialog-button--discard';
    ...
    function show(options) {
        ...
        var messageText = options.messageText || 'You have unsaved changes. Save before closing?';
        var cancelLabel = options.cancelLabel || 'Cancel';
        var saveLabel = options.saveLabel || 'Save';
        var discardLabel = options.discardLabel || 'Discard Changes';
        var onCancel = options.onCancel || function() {};
        var onSave = options.onSave || function() {};
        var onDiscard = options.onDiscard || function() {};
        var discardDisabled = options.discardDisabled !== false; // Default to disabled
        
        var overlay = ensureOverlay();
        var title = overlay.querySelector('#unsavedDialogTitle');
        var message = overlay.querySelector('#unsavedDialogMessage');
        var cancelBtn = overlay.querySelector('[data-role="cancel"]');
        ...
        discardBtn.disabled = discardDisabled;
        
        // Show overlay
        overlay.classList.add('component-unsaved-dialog-overlay--visible');
        overlay.setAttribute('aria-hidden', 'false');
        ...
        return new Promise(function(resolve) {
            function cleanup() {
                overlay.classList.remove('component-unsaved-dialog-overlay--visible');
                overlay.setAttribute('aria-hidden', 'true');
                document.removeEventListener('keydown', onKeyDown, true);
                overlay.removeEventListener('click', onOverlayClick);
            ...
            function onKeyDown(e) {
                if (e.key === 'Escape') {
                    ...
                    if (focused === cancelBtn) {
                        ...
                        onSave();
                        resolve('save');
                    } else if (focused === discardBtn && !discardBtn.disabled) {
                        cleanup();
                        onDiscard();
                        resolve('discard');
                    }
                    ...
                    // Trap focus inside dialog
                    var buttons = [cancelBtn, saveBtn, discardBtn].filter(function(btn) {
                        return !btn.disabled;
                    });
                    var first = buttons[0];
                    var last = buttons[buttons.length - 1];
